...
foundation_build() {
  ...
  dotnet build "${slnf}" --no-restore --configuration "${CONFIGURATION}" /p:RunAnalyzers=false || rc=$?
  echo "::endgroup::"
  rm -f "${slnf}"
  return "${rc}"
}
...
foundation_run() {
  ...
  while IFS='|' read -r csproj trx filter envs flags; do
    ...
    if [[ ",${flags}," == *,advisory,* ]]; then
      ...
      dotnet build "${csproj}" --no-restore --configuration "${CONFIGURATION}" /p:RunAnalyzers=false || build_rc=$?
      echo "::endgroup::"
      if ((build_rc != 0)); then
