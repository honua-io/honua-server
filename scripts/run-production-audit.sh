    ...
    if phase_selected "2"; then
        ...
        if [[ "$SKIP_CITE" == "true" ]]; then
            ...
            run_check "protocol" "2" "cite-ogc-features" "required" \
                "./scripts/run-cite-tests.sh --profile $cite_profile" || return $?
            run_check "protocol" "2" "cite-ogc-tiles" "required" \
                "./scripts/run-cite-tiles-tests.sh --profile $cite_profile" || return $?
            run_check "protocol" "2" "cite-wms-13" "required" \
                "./scripts/run-cite-wms-tests.sh --profile $cite_profile" || return $?
            run_check "protocol" "2" "cite-wmts-10" "required" \
                "./scripts/run-cite-wmts-tests.sh --profile $cite_profile" || return $?
        fi
    fi
