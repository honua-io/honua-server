...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix=project) as directory:
        ...
        def status(path, *, proxy=False, authenticated=False, host=None):
            headers = {}
            if host:
                headers["Host"] = host
            if authenticated:
        ...
        try:
            if compose("up", "-d", "--wait", "--wait-timeout", str(args.timeout), "postgres", "redis").returncode:
                ...
                if compose("up", "-d", "--no-deps", "honua").returncode:
                    ...
                    if receipt["ready"]:
                        proxy_started = compose("up", "-d", "--no-deps", "smoke_proxy").returncode == 0
                        deadline = time.monotonic() + 30
                        while proxy_started and time.monotonic() < deadline:
