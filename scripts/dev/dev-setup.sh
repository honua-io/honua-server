...
setup_project_env() {
    ...
    # Setup git hooks
    if [ -f "scripts/dev/setup-git-hooks.sh" ]; then
        echo "🪝 Setting up git hooks..."
        bash scripts/dev/setup-git-hooks.sh
    fi

    # Create .env files for development
    if [ -f "scripts/security/secret-management.sh" ]; then
        echo "🔐 Generating development secrets..."
        ENV_TYPE=development SECRET_BACKEND=file bash scripts/security/secret-management.sh generate
    fi
...
show_next_steps() {
    ...
    echo "4. Start development services: python3 scripts/docker/quickstart.py"
    echo "5. Run the application: cd src/Honua.Server && dotnet run"
    echo ""
    echo "Useful commands:"
    echo "- Start services: python3 scripts/docker/quickstart.py"
    echo "- View logs: docker compose logs -f"
    echo "- Stop services: docker compose down"
    echo "- Run tests: dotnet test"
    echo "- Check format: dotnet format Honua.sln --verify-no-changes"
    echo "- Pre-PR check: ./scripts/ci/pre-pr-check.sh"
    echo ""
    echo "Development URLs (when running):"
    echo "- Application: http://localhost:8080"
    echo "- Swagger UI: http://localhost:8080/swagger"
    echo "- Health checks: http://localhost:8080/healthz/live"
    echo "- PostgreSQL: localhost:5432 (user: honua_user; password generated in .env by quickstart)"
    echo ""
    echo "For help, see README.md or run: ./scripts/dev/dev-setup.sh --help"
}
...
main() {
    ...
    if validate_installation; then
        ...
        echo "You can run './scripts/dev/dev-setup.sh --validate-only' to check again."
        exit 1
    fi
}
