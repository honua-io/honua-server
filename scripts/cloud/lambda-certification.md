...
| `REALAWS_CERT_DENIED_KEY` cert secret | **Optional override, deprecated.** The lane mints its own scoped `read:layers` principal per run (see below) and no longer needs this secret. Supplying it keeps an existing bootstrap working for one release: the lane then sends that key instead of minting one, and mints and revokes nothing. Remove the secret once this release has shipped. |
...
The key it sent came from `REALAWS_CERT_DENIED_KEY`, minted into the cert database at bootstrap on
2026-09-06. **An admin API key is a database row**, not configuration: it lives in the cert
PostGIS the standing function migrates and the bootstrap re-seeds. That row did not survive the
migration and re-seed the cert database had since been through, so the key almost certainly no
longer authenticated at all — and a key that does not authenticate answers 401, which certifies a
missing credential rather than authorization.
...
- Every phase (`deployed`, `baseline`, `candidate`, `rollback`) sends that key for the denial
  assertion. All four run against the one cert database, so one row serves the whole run —
  including the baseline and rollback phases, which validate the candidate's row with the
...
- Teardown revokes it by that unique name — which also recovers the row a lost create response left
...
non-admin principal (200 — honua-server#4386). A failed denial assertion now prints a second line:

```
serving-assertion: phase=deployed path=/api/v1/admin/api-keys status=401 expected=403 body-kind=json error=Unauthorized
serving-403: phase=deployed principal=override key=HONUA_LAMBDA_CERT_DENIED_KEY status=401 body-kind=json authenticated=no challenge=ApiKey+Basic records=0 record=unknown detail=API key required. Provide a valid API key in the X-API-Key header.
```
...
| `authenticated` | Whether the server recognized the principal at all: `no` for 401, `yes` for 403 or 200. This is the field that separates a missing key from a leak. |
| `challenge` | The scheme that issued the `WWW-Authenticate` challenge, parsed as in the 401 diagnostic above. Present only when the server challenged. |
...
- `serving.deniedKey`: `source`, the granted `permissions`, this run's key `name` (never its
  ...
  zero. An `override` run creates and revokes nothing.
