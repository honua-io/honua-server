...
CITE_COMPOSE_FILE="docker/cite/wfs10/compose.yml"
CITE_RESULTS_DIR="cite-wfs10-results"
CITE_TIMEOUT=1800
HEALTHCHECK_TIMEOUT=300
HONUA_CITE_WFS10_SERVER_PORT="${HONUA_CITE_WFS10_SERVER_PORT:-8096}"
HONUA_BASE_URL="http://localhost:${HONUA_CITE_WFS10_SERVER_PORT}"
HONUA_WFS_URL="${HONUA_BASE_URL}/wfs?service=WFS&version=1.0.0&request=GetCapabilities"
HONUA_CITE_WFS10_POSTGRES_PORT="${HONUA_CITE_WFS10_POSTGRES_PORT:-5446}"
export HONUA_CITE_WFS10_SERVER_PORT
export HONUA_CITE_WFS10_POSTGRES_PORT

echo -e "${BLUE}OGC WFS 1.0 CITE Conformance Tests${NC}"
echo "===================================="
...
echo -e "${YELLOW}Starting WFS 1.0 CITE test environment...${NC}"
$COMPOSE_CMD -f "$CITE_COMPOSE_FILE" down --remove-orphans --volumes 2>/dev/null || true
...
if [[ "$INTERACTIVE" == "true" ]]; then
    ...
    echo "CITE Team Engine:    http://localhost:8088/teamengine"
    tail -f /dev/null
fi
...
if [[ $TEST_EXIT_CODE -eq 124 ]]; then
    echo -e "${RED}WFS 1.0 CITE tests timed out after ${CITE_TIMEOUT}s${NC}"
    exit 1
fi

if [[ $TEST_EXIT_CODE -eq 0 ]]; then
    echo -e "${GREEN}WFS 1.0 CITE tests completed successfully${NC}"
else
    echo -e "${RED}WFS 1.0 CITE tests failed with exit code $TEST_EXIT_CODE${NC}"
fi
