...
class FragmentTests(unittest.TestCase):
    def build(self, *, include_suite=True, image_digest=DIGEST, passed=167,
              fixture_revision="docker/cite/shared/test-data@{source_sha}",
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            summary = {"suites": ([{"id": "wfs20", "status": "passed", "totalTests": 167,
                                    "passed": passed, "failed": 0, "skipped": 0, "cantTell": 0}]
    ...
    def test_inconsistent_pass_counts_emit_failure(self):
        observation = self.build(passed=166)["observations"][0]
        self.assertEqual("fail", observation["result"])

    def test_mismatched_governed_fixture_is_explicit_skip(self):
        observation = self.build(
            fixture_revision="docker/cng/seed.sql@{source_sha}"
        )["observations"][0]
        self.assertEqual("skip", observation["result"])
        self.assertIn("was not exercised", observation["skip_reason"])
