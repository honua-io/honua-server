...
CITE_COMPOSE_FILE="docker/cite/wmts10/compose.yml"
CITE_RESULTS_DIR="cite-wmts-results"
...
WMTS_INCLUDE_GETTILE_TESTS="on"
WMTS_INCLUDE_GETFEATUREINFO_TESTS="off"
WMTS_INCLUDE_KVP_GET_ENCODING="on"
WMTS_INCLUDE_RESTFUL_ENCODING="off"
...
set_profile_options() {
    ...
    case "$profile" in
        minimal)
            WMTS_INCLUDE_GETTILE_TESTS="off"
            ;;
        default)
            ;;
        full)
            WMTS_INCLUDE_GETFEATUREINFO_TESTS="on"
            WMTS_INCLUDE_RESTFUL_ENCODING="on"
            ;;
        *)
...
echo -e "${BLUE}WMTS 1.0 CITE Conformance Tests${NC}"
echo "================================="
...
echo -e "${YELLOW}Starting CITE WMTS test environment...${NC}"
$COMPOSE_CMD -f "$CITE_COMPOSE_FILE" down --remove-orphans --volumes 2>/dev/null || true
...
echo -e "${YELLOW}Seeding CITE WMTS database...${NC}"
...
if ! curl -s -f "$GETTILE_URL" > /dev/null; then
    echo -e "${YELLOW}WMTS GetTile preflight failed; continuing to CITE execution for full diagnostics${NC}"
fi

echo -e "${GREEN}WMTS endpoints are accessible${NC}"
...
echo -e "${YELLOW}Capturing WMTS capabilities...${NC}"
if ! curl -s -f "$CAPS_URL_HOST" > "$CITE_RESULTS_DIR/capabilities.xml"; then
    echo -e "${RED}Failed to capture WMTS capabilities${NC}"
    exit 1
fi

set_profile_options "$PROFILE"

echo -e "${YELLOW}Running WMTS 1.0 CITE conformance tests (profile: $PROFILE)...${NC}"
$COMPOSE_CMD -f "$CITE_COMPOSE_FILE" rm -f -s cite-runner >/dev/null 2>&1 || true
WMTS_CAPABILITIES_URL="$CAPS_URL_CONTAINER" \
WMTS_FAIL_EARLY="$WMTS_FAIL_EARLY" \
WMTS_INCLUDE_COMMON_OPERATIONS_TESTS="$WMTS_INCLUDE_COMMON_OPERATIONS_TESTS" \
WMTS_INCLUDE_GETCAPABILITIES_TESTS="$WMTS_INCLUDE_GETCAPABILITIES_TESTS" \
WMTS_INCLUDE_GETTILE_TESTS="$WMTS_INCLUDE_GETTILE_TESTS" \
WMTS_INCLUDE_GETFEATUREINFO_TESTS="$WMTS_INCLUDE_GETFEATUREINFO_TESTS" \
WMTS_INCLUDE_KVP_GET_ENCODING="$WMTS_INCLUDE_KVP_GET_ENCODING" \
WMTS_INCLUDE_RESTFUL_ENCODING="$WMTS_INCLUDE_RESTFUL_ENCODING" \
...
if [[ -d "$CITE_RESULTS_DIR" && $(ls -A "$CITE_RESULTS_DIR" 2>/dev/null) ]]; then
    ...
    if [[ -n "$RESULTS_XML" && -f "$RESULTS_XML" ]]; then
        ...
        SESSION_DIR=$(find "$CITE_RESULTS_DIR" -maxdepth 1 -type d -name "cite-wmts-session-*" | sort | tail -n 1)
        RESULT_CODE_LINES=""
...
cat > "$CITE_RESULTS_DIR/cite-wmts-summary.md" << EOF_SUMMARY
# WMTS 1.0 CITE Conformance Test Results
...
- **Profile**: $PROFILE
- **CITE Suite**: ets-wmts10
- **Capabilities URL**: $CAPS_URL

## Artifacts

- capabilities.xml: captured WMTS capabilities document
- testng-results.xml: raw TeamEngine result summary (when available)

EOF_SUMMARY

echo -e "${GREEN}Summary report saved to: $CITE_RESULTS_DIR/cite-wmts-summary.md${NC}"
