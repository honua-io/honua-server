...
/// <summary>
/// Builds the constrained JSON schema for the report-generation structured output, mirroring
/// <c>FormGenerationSchema</c>. Constrains each panel's <c>kind</c> to the supported panel-kind vocabulary
/// so the model cannot invent panel kinds. The <c>chartSpec</c> is a free-form Vega-Lite object — its inner
/// shape is intentionally unconstrained so the model can emit any valid Vega-Lite mark/encoding.
/// </summary>
internal static class ReportGenerationSchema
{
    public static JsonElement Build()
    {
        ...
        var panelKindEnum = string.Join(",",
            ReportGenerationPrompt.PanelKinds.Select(k => "\"" + JsonEncodedText.Encode(k) + "\""));
