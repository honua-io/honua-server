...
internal sealed class SpatialReferenceResolver
{
    ...
    {
        // First try to resolve from explicit SR value
    ...
    public async Task<int?> ParseSridAsync(string? srValue, CancellationToken cancellationToken = default)
    {
        ...
        // Try parsing as JSON spatial reference
        if (trimmed.StartsWith('{'))
        {
            return await ParseSpatialReferenceJsonAsync(trimmed, cancellationToken);
        }
        ...
        // Try WKT detection if it looks like WKT
        if (LooksLikeWkt(trimmed))
        {
            return await _crsDetectionService.DetectFromWktAsync(trimmed);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return await _crsDetectionService.DetectFromWktAsync(spatialRef.Wkt);
        }
    ...
    private async Task<int?> ParseSpatialReferenceJsonAsync(string jsonValue, CancellationToken cancellationToken)
    {
        ...
        // Try WKT-based detection
        if (!string.IsNullOrWhiteSpace(parseResult.Wkt))
        {
            return await _crsDetectionService.DetectFromWktAsync(parseResult.Wkt);
        }
