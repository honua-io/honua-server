...
internal static class OgcFeatureMutationHelpers
{
    ...
    /// <see cref="TryBuildFeatureAsync(HttpRequest, LayerDefinition, GeoJsonFeature, ICrsRegistry, OgcFeaturesGeometryServices, FeatureMutationValidator, long, CancellationToken)"/>.
    /// Resolves storage SRID, primary geometry field, and attribute validation
    /// from the V2 canonical resource. <paramref name="isUpdate"/> distinguishes
    /// create- vs update-path attribute validation (non-editable fields are
    /// rejected on update).
    ...
    /// <summary>
    /// Metadata v2 overload of
    /// <see cref="TryBuildFeatureAsync(LayerDefinition, GeoJsonFeature, CrsDefinition, OgcFeaturesGeometryServices, FeatureMutationValidator, long, CancellationToken)"/>.
    /// </summary>
    public static async Task<FeatureBuildResult> TryBuildFeatureAsync(
        MetadataV2Resource resource,
        GeoJsonFeature requestFeature,
