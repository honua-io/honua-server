...
public sealed class RedisOperationSecretStore : IOperationSecretStore
{
    ...
    // The entry and its expiry must land together: a hash that outlived a failed follow-up
    // KeyExpire would keep a decryptable credential in Redis forever.
    private const string StoreScript = """
        redis.call('HSET', KEYS[1],
            'operationInstanceId', ARGV[1],
            'operationId', ARGV[2],
            'principalId', ARGV[3],
            'tenantId', ARGV[4],
            'name', ARGV[5],
            'protectedValue', ARGV[6])
