...
/// <summary>
/// In-memory <see cref="IRlsPolicyStore"/> default (#502). Registered via TryAdd so a
/// durable provider implementation (e.g. <c>PostgresRlsPolicyStore</c>) wins when
/// present; keeps non-Postgres deployments and tests functional without persistence.
/// </summary>
internal sealed class InMemoryRlsPolicyStore : IRlsPolicyStore
{
    private readonly ConcurrentDictionary<Guid, RlsPolicy> _policies = new();

    public Task<IReadOnlyList<RlsPolicy>> ListPoliciesAsync(CancellationToken cancellationToken = default)
    {
        IReadOnlyList<RlsPolicy> result = _policies.Values
    ...
    public Task<RlsPolicy> CreatePolicyAsync(RlsPolicy policy, CancellationToken cancellationToken = default)
    {
        ...
        var duplicate = _policies.Values.Any(existing =>
            ...
            string.Equals(existing.ClaimType, policy.ClaimType, StringComparison.OrdinalIgnoreCase));
        if (duplicate)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        IReadOnlyList<RlsPolicy> matches = _policies.Values
            .Where(p =>
