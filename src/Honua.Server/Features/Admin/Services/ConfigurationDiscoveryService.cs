...
public sealed class ConfigurationDiscoveryService
{
    ...
    private static DateTimeOffset GetLastModifiedTime(Type type)
    {
        ...
        catch
        {
            ...
            // actionable and doesn't need logging (static helper has no logger).
    ...
    {
        return types.Select(typeInfo => new AutoGeneratedDocumentationSection
        {
            SectionName = typeInfo.SectionName,
            TypeName = typeInfo.TypeName,
    ...
    private List<string> ScanForSecretReferences()
    {
        ...
        return _configuration.AsEnumerable()
            .Where(section => !string.IsNullOrEmpty(section.Value) && IsLikelySecretReference(section.Value))
...
internal static partial class ConfigurationDiscoveryLog
{
    ...
    [LoggerMessage(
        EventId = 20007,
        Level = LogLevel.Warning,
        Message = "Failed to read property {PropertyName} of {TypeName} via reflection")]
    public static partial void PropertyValueReadFailed(ILogger logger, string typeName, string propertyName, Exception exception);

    [LoggerMessage(
        EventId = 20008,
        Level = LogLevel.Warning,
        Message = "Secret reference validation failed for {SecretReference}")]
    public static partial void SecretReferenceValidationFailed(ILogger logger, string secretReference, Exception exception);

    [LoggerMessage(
        EventId = 20009,
        Level = LogLevel.Error,
        Message = "Secret validation scan failed")]
    public static partial void SecretValidationFailed(ILogger logger, Exception exception);
}
