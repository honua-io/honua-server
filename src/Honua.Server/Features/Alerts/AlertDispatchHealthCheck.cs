...
internal sealed class AlertDispatchHealthCheck : IHealthCheck
{
    ...
    internal HealthCheckResult Evaluate(DateTimeOffset now)
    {
        ...
        // Heartbeat staleness: the dispatcher reports "running" but its last poll aged past the
        // threshold, so the loop is wedged inside a pass while claiming to be healthy — the exact
        // failure a plain running-flag check misses. LastPollAt is null only before the first pass
        // (fresh start), which is not a fault.
        if (_dispatcherHealth.LastPollAt is { } pollAt
            && now - pollAt >= _options.Dispatch.HeartbeatStalenessThreshold)
        {
            return HealthCheckResult.Unhealthy(
                $"Alert dispatcher heartbeat is stale (last poll {pollAt.ToString("o", CultureInfo.InvariantCulture)}); the loop appears hung and notifications are not draining.",
                data: data);
        }
        ...
        return HealthCheckResult.Healthy("Alert dispatcher is healthy.", data);
    }
}
