...
internal static class ProcessEndpoints
{
        ...
        [FromServices] IExecutionJobStore? jobStore = null)
    {
        ...
        {
            OgcProcessesLog.AuthorizationDenied(logger, OperatorResourceType.Process.ToString(), OperatorOperation.Execute.ToString());
        ...
        // Only emit an absolute Location header when an explicit public base URL is configured.
        // Falling back to a relative Location is RFC 9110 §10.2.2 / OGC API Processes compliant
        // and avoids reflecting untrusted request-derived host data into a redirect target.
        var jobPath = $"{BasePath}/jobs/{jobId}";
        context.Response.Headers["Location"] = BaseUrlResolver.TryGetConfiguredBaseUrl(context, out var configuredBaseUrl)
            ? $"{configuredBaseUrl}{jobPath}"
            : jobPath;
    ...
    internal static IResult FormatOgcApprovalError(ApprovalRequirement approval) => Results.Json(
        new OgcProcessError
        {
            Type = "about:blank",
