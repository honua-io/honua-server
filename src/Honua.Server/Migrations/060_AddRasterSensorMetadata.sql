...
DO $$
BEGIN
    IF to_regclass('$HonuaSchema$.raster_data') IS NULL THEN
        RAISE NOTICE '$HonuaSchema$.raster_data not present; skipping raster_sensor_metadata creation (provisioned with the raster schema).';
        RETURN;
    END IF;

    CREATE TABLE IF NOT EXISTS $HonuaSchema$.raster_sensor_metadata (
        raster_data_id BIGINT PRIMARY KEY
            REFERENCES $HonuaSchema$.raster_data(id) ON DELETE CASCADE,
    ...
    COMMENT ON TABLE $HonuaSchema$.raster_sensor_metadata IS
    ...
    COMMENT ON COLUMN $HonuaSchema$.raster_sensor_metadata.sensor_name IS 'Human-readable sensor name (e.g. WorldView-3).';
    COMMENT ON COLUMN $HonuaSchema$.raster_sensor_metadata.camera_model IS 'Camera/instrument model identifier.';
    COMMENT ON COLUMN $HonuaSchema$.raster_sensor_metadata.interior_orientation IS 'Interior orientation (focal length, principal point, distortion) as JSON.';
    COMMENT ON COLUMN $HonuaSchema$.raster_sensor_metadata.exterior_orientation IS 'Exterior orientation (camera position, look vector, nadir point, off-nadir angle) as JSON.';
    COMMENT ON COLUMN $HonuaSchema$.raster_sensor_metadata.rpc IS 'Rational Polynomial Coefficients image-to-ground model as JSON.';
    COMMENT ON COLUMN $HonuaSchema$.raster_sensor_metadata.dem_source IS 'DEM source (layer id or named source) used for base/top elevation differencing.';
END
$$;
