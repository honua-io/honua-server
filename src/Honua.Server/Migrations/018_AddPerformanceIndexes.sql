...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_layers_id_created
ON honua.layers (layer_id, created_at);

-- Composite index for layer publishing queries (layer_id, updated_at)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_layers_id_updated
ON honua.layers (layer_id, updated_at);

-- Optimize layer lookup queries by schema and table
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_layers_schema_table
ON honua.layers (table_schema, table_name);

-- Index for enabled layer filtering
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_layers_enabled
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_features_geometry_nn
ON features USING GIST (geometry) WHERE geometry IS NOT NULL;

-- Specialized index for WGS84 transformations (caches common transforms)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_features_geom_4326
ON features USING GIST (ST_Transform(geometry, 4326))
WHERE geometry IS NOT NULL AND ST_SRID(geometry) != 4326;

-- 3D spatial index for elevation/Z-dimension queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_features_geometry_3d
ON features USING GIST (geometry gist_geometry_ops_nd)
WHERE ST_NDims(geometry) > 2;

-- Envelope/bbox optimization index
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_features_envelope
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_features_attr_dates
ON features USING BTREE ((attributes ->> 'created_date')::date)
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_features_attr_timestamps
ON features USING BTREE ((attributes ->> 'updated_at')::timestamptz)
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_features_temporal_attrs
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_layer_fields_layer_order
ON honua.layer_fields (layer_id, field_order);

-- Index for field type filtering
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_layer_fields_type
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_service_layers_order
...
COMMENT ON INDEX idx_layers_id_created IS 'Optimizes layer publishing queries by layer_id and creation time';
COMMENT ON INDEX idx_layers_id_updated IS 'Optimizes layer publishing queries by layer_id and update time';
...
COMMENT ON INDEX idx_features_attr_dates IS 'Optimizes date attribute queries with proper casting';
COMMENT ON VIEW honua.index_usage IS 'Monitors database index utilization for performance tuning';
COMMENT ON VIEW honua.slow_feature_queries IS 'Identifies slow queries on features table for optimization';