...
CREATE TABLE IF NOT EXISTS $HonuaSchema$.studio_map_comment_threads (
...
CREATE INDEX IF NOT EXISTS idx_studio_map_comment_threads_map
    ON $HonuaSchema$.studio_map_comment_threads (map_id, updated_at DESC);

CREATE TABLE IF NOT EXISTS $HonuaSchema$.studio_map_comment_messages (
    message_id  UUID        NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    thread_id   UUID        NOT NULL REFERENCES $HonuaSchema$.studio_map_comment_threads(thread_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_studio_map_comment_messages_thread
    ON $HonuaSchema$.studio_map_comment_messages (thread_id, created_at);

CREATE TABLE IF NOT EXISTS $HonuaSchema$.studio_map_activity_events (
...
CREATE INDEX IF NOT EXISTS idx_studio_map_activity_events_map
    ON $HonuaSchema$.studio_map_activity_events (map_id, occurred_at DESC);

COMMENT ON TABLE $HonuaSchema$.studio_map_comment_threads IS
    'Feature-pinned comment threads on a Studio map draft/package (#1278, slice 1).';
COMMENT ON TABLE $HonuaSchema$.studio_map_activity_events IS
