...
CREATE INDEX IF NOT EXISTS idx_studio_content_items_list
    ON $HonuaSchema$.studio_content_items (updated_at DESC, item_id DESC);

CREATE INDEX IF NOT EXISTS idx_studio_content_items_family_list
    ON $HonuaSchema$.studio_content_items (family, updated_at DESC, item_id DESC);

CREATE INDEX IF NOT EXISTS idx_studio_content_items_creator_list
    ON $HonuaSchema$.studio_content_items (created_by, updated_at DESC, item_id DESC)
...
CREATE INDEX IF NOT EXISTS idx_studio_package_drafts_list
    ON $HonuaSchema$.studio_package_drafts (updated_at DESC, draft_id DESC);

CREATE INDEX IF NOT EXISTS idx_studio_package_drafts_family_list
    ON $HonuaSchema$.studio_package_drafts (family, updated_at DESC, draft_id DESC);

CREATE INDEX IF NOT EXISTS idx_studio_package_drafts_owner_list
    ON $HonuaSchema$.studio_package_drafts (owner_id, updated_at DESC, draft_id DESC)
    WHERE owner_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_studio_package_drafts_workspace_list
    ON $HonuaSchema$.studio_package_drafts (workspace_id, updated_at DESC, draft_id DESC)
...
CREATE INDEX IF NOT EXISTS idx_content_publication_versions_source_content
    ON $HonuaSchema$.content_publication_versions (source_content_id, created_at DESC, revision DESC)
