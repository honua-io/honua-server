...
namespace Honua.Snowflake.Features.FeatureStore;

/// <summary>
/// Read-only Snowflake feature provider for the spatial provider thin slice (#1713).
/// </summary>
/// <remarks>
/// <para>Implements the canonical <see cref="IFeatureDataProvider"/> + <see cref="IFeatureReader"/>
/// surface for layers backed by Snowflake native <c>GEOGRAPHY</c>/<c>GEOMETRY</c> columns. Edits,
/// native MVT, native FlatGeobuf/Geobuf/GML, statistics aggregates, top-features, bins, and H3 are
...
/// </remarks>
internal sealed class SnowflakeFeatureStore : IFeatureDataProvider, IFeatureReader, IBindableFeatureDataProvider
{
    ...
    /// <inheritdoc />
    public string ProviderName => DataProviderNames.Snowflake;
    ...
    public async Task<Feature?> GetAsync(int layerId, long featureId, CancellationToken cancellationToken = default)
    {
        ...
        var sql = SnowflakeFeatureQueryBuilder.BuildSelectQuery(mapping, query, attributeColumns);
    ...
    public async Task<QueryResult<Feature>> QueryAsync(int layerId, FeatureQuery query, CancellationToken cancellationToken = default)
    {
        ...
        var sql = SnowflakeFeatureQueryBuilder.BuildSelectQuery(mapping, probeQuery, attributeColumns);
        var features = await _dataAccess.ExecuteSelectAsync(mapping, sql, attributeColumns, _boundConnection, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<ImmutableArray<long>> QueryObjectIdsAsync(int layerId, FeatureQuery query, CancellationToken cancellationToken = default)
    {
        ...
        var sql = SnowflakeFeatureQueryBuilder.BuildObjectIdsQuery(mapping, query);
    ...
    {
        var (mapping, _) = await ResolveLayerAsync(layerId, cancellationToken).ConfigureAwait(false);
        var sql = SnowflakeFeatureQueryBuilder.BuildCountQuery(mapping, query);
    ...
    {
        var (mapping, _) = await ResolveLayerAsync(layerId, cancellationToken).ConfigureAwait(false);
        var sql = SnowflakeFeatureQueryBuilder.BuildExtentQuery(mapping, query);
    ...
    public async Task<EstimateResult> GetEstimatesAsync(int layerId, CancellationToken cancellationToken = default)
    {
        ...
        var countTask = _dataAccess.ExecuteCountAsync(mapping, SnowflakeFeatureQueryBuilder.BuildCountQuery(mapping, emptyQuery), _boundConnection, cancellationToken);
        var extentTask = _dataAccess.ExecuteExtentAsync(mapping, SnowflakeFeatureQueryBuilder.BuildExtentQuery(mapping, emptyQuery), _boundConnection, cancellationToken);
    ...
    private static NotSupportedException NotSupported(string operation, int layerId)
        => new($"Snowflake provider does not support '{operation}' for layer {layerId} in this slice.");
}
