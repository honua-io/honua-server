    ...
    [
        ...
        new ProcessDefinition
        {
            ...
            [
                ...
                Param("bbox", "BBox", "Bounding-box filter applied to the source layer (layerId source only), as minx,miny,maxx,maxy expressed in 'outSrid' (the connector stamps the envelope with the requested output SRID), so a non-default outSrid requires bbox coordinates in that same CRS.", ProcessParameterValueType.Text),
                Param("outSrid", "Output SRID", "EPSG code both the source and dataset layers are streamed in, so the join compares comparable ordinates. Defaults to 4326; choose a metric CRS (e.g. 3857) when 'distance'/NEAR_DIST must be in meters. Must be 4326 when the source is supplied inline via 'input' (inline GeoJSON is WGS 84 by specification).", ProcessParameterValueType.WholeNumber, defaultValue: "4326"),
