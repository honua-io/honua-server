...
internal sealed class BuiltInProcessCatalog : IProcessCatalog
{
    ...
    [
        ...
        new ProcessDefinition
        {
            ...
            Description = "Runs supervised/unsupervised classification, segmentation, or object detection on a raster scene by DELEGATING inference to a configured cloud backend (Geoprocessing:ImageryInference — generic 'http' REST adapter for OpenAI-compatible / hosted-ONNX / Azure ML online endpoints; 'sagemaker'/'vertex'/'azureml' SDK adapters are recognized but not yet supported and fail clearly). The model is a reference into the backend, never a Honua-implemented algorithm. Reads a base64-encoded GeoTIFF from 'source' (or a layerId/rasterId resolved at submit time) and publishes either a classification/segmentation GeoTIFF (georeferencing preserved byte-for-byte from the backend and verified against the source extent/CRS) or detected features as a GeoJSON FeatureCollection in WGS 84 (EPSG:4326) longitude/latitude per RFC 7946. The source must itself be a georeferenced GeoTIFF so the output location can be verified. When no backend is configured the process is advertised but every execution fails with a clear 'no cloud inference backend is configured' message — no silent stub, no fake result.",
            Category = "imagery",
            Parameters =
            [
                .. NativeRasterSourceParameters,
