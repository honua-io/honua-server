    ...
    [
        ...
        new ProcessDefinition
        {
            ...
            [
                ...
                Param("unit", "Unit", "Distance unit. Allowed values: meters (default), kilometers, feet, miles. The chosen unit is converted to meters and applied as planar CRS units; geographic (degree) inputs are rejected (a meters-as-degrees buffer is meaningless) — project to a metric CRS first. No geodesic conversion is performed.", ProcessParameterValueType.Text, defaultValue: "meters"),
                Param("srid", "Input CRS (SRID)", "SRID/WKID the input geometries are in, used to validate the buffer unit against the CRS. Defaults to the input geometry SRID, else WGS 84 (4326). A linear-unit buffer against a geographic (degree) CRS is rejected — reproject to a metric CRS (e.g. EPSG:3857) or set this to the projected CRS the geometries are actually in.", ProcessParameterValueType.WholeNumber),
