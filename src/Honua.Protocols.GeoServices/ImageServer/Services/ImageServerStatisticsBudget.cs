...
/// <summary>
/// Bounds the cost of computing (and persisting) raster mosaic statistics so a large,
/// cold-cache mosaic degrades gracefully within a fixed, short time budget instead of
/// blocking the request until an outer host-level deadline (a serverless function
/// timeout, a load-balancer idle timeout, ...) kills the whole connection with an
/// opaque, unbounded hang (#2991).
/// </summary>
/// <remarks>
...
/// fast, predictable response (metadata without statistics) on the first few calls while
/// the persisted-stats backfill keeps a chance to complete out from under the tighter
/// budget on a request that happens to hit a warm cache.
/// </remarks>
internal static class ImageServerStatisticsBudget
{
    /// <summary>
    /// Maximum time a single request will wait for raster statistics before falling back
    /// to an empty result. Comfortably below the platform's default request/function
    /// timeouts (60s) so the response always returns well inside typical client and
    /// infrastructure timeouts instead of racing them.
    /// </summary>
    ...
    /// statistics array is returned instead of propagating the cancellation — the caller
    /// still gets a valid (if statistics-less) response rather than an exception. A
    /// client disconnect (<paramref name="requestAborted"/> itself firing) is not treated
    /// as a budget timeout and still propagates as a normal cancellation.
    /// </summary>
    public static Task<RasterStatistics[]> ResolveAsync(
        Func<CancellationToken, Task<RasterStatistics[]>> operation,
        Action onBudgetExceeded,
        CancellationToken requestAborted)
        => ResolveAsync(operation, onBudgetExceeded, Timeout, requestAborted);

    /// <summary>
    /// Test seam: identical to <see cref="ResolveAsync(Func{CancellationToken,Task{RasterStatistics[]}},Action,CancellationToken)"/>
    ...
    /// </summary>
    internal static async Task<RasterStatistics[]> ResolveAsync(
        Func<CancellationToken, Task<RasterStatistics[]>> operation,
        Action onBudgetExceeded,
        TimeSpan budget,
        CancellationToken requestAborted)
