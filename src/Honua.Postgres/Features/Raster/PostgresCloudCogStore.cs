...
/// <summary>
/// PostgreSQL-based cloud COG catalog store.
/// Persists cloud COG registrations and cached metadata.
/// </summary>
internal sealed class PostgresCloudCogStore : ICloudCogStore
{
    private readonly IDatabaseConnectionProvider _connectionProvider;
    private readonly ILogger<PostgresCloudCogStore> _logger;
    private readonly string _tableName;

    public PostgresCloudCogStore(
        IDatabaseConnectionProvider connectionProvider,
        ILogger<PostgresCloudCogStore> logger,
    ...
    /// <inheritdoc />
    public async Task<CloudCogRegistration?> GetAsync(long id, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public async Task<CloudCogRegistration> RegisterAsync(CloudCogRegistrationRequest request, CancellationToken cancellationToken = default)
    {
        await using var connection = await _connectionProvider.OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
        await using var command = connection.CreateCommand();
        command.CommandText = $"""
        ...
        try
        {
            ...
            if (!await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                throw new InvalidOperationException("Cloud COG registration INSERT did not return the expected row.");
            }

            return new CloudCogRegistration
            {
    ...
    /// <inheritdoc />
    public async Task<CloudCogRegistration[]> ListByLayerAsync(int layerId, CancellationToken cancellationToken = default)
    {
        await using var connection = await _connectionProvider.OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
        await using var command = connection.CreateCommand();
        command.CommandText = $"""
        ...
        var results = new List<CloudCogRegistration>();
        await using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
    ...
    private static CloudCogRegistration ReadRegistration(System.Data.Common.DbDataReader reader)
    {
