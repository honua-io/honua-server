        ...
        CoreGeometryStorageType geometryStorageType = CoreGeometryStorageType.Geometry)
    {
        ...
        try
        {
            ...
            if (query.RasterPointGrid is { } pointGrid)
            {
                ...
                sql.Append("FROM projected_points) SELECT AVG(x)::double precision AS x, AVG(y)::double precision AS y FROM snapped_points GROUP BY cell_x, cell_y ORDER BY cell_x, cell_y");
            }
            else
            {
                sql.Append(CultureInfo.InvariantCulture,
