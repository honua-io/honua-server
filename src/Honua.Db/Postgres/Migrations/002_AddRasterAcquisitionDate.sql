...
ALTER TABLE IF EXISTS $HonuaSchema$.raster_data
    ADD COLUMN IF NOT EXISTS acquisition_date TIMESTAMPTZ;

CREATE INDEX IF NOT EXISTS idx_raster_data_acquisition_date
    ON $HonuaSchema$.raster_data(acquisition_date);

CREATE INDEX IF NOT EXISTS idx_raster_data_layer_acquisition
    ON $HonuaSchema$.raster_data(layer_id, acquisition_date DESC, created_at DESC, id DESC);

COMMENT ON COLUMN $HonuaSchema$.raster_data.acquisition_date
