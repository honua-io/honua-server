...
public static class EdrEndpoints
{
    ...
    public static void MapEdrEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/collections/{collectionId}/position", GetPosition)
            ...
            .WithDescription("Returns a CoverageJSON time-series at coords=POINT(lon lat), or a CoverageJSON CoverageCollection with one time-series per position for coords=MULTIPOINT((lon lat),(lon lat)). Optional: datetime, parameter-name, f (CoverageJSON), crs (CRS84); any other f or crs value returns 400. Returns 204 when datetime does not intersect the collection's temporal extent.")
        ...
        group.MapGet("/collections/{collectionId}/cube", GetCube)
            ...
            .WithDescription("Returns a CoverageJSON grid subset for bbox=minLon,minLat,maxLon,maxLat. Optional: datetime, parameter-name, resolution-x, f (CoverageJSON), crs (CRS84); any other f or crs value returns 400. Returns 204 when datetime does not intersect the collection's temporal extent.")
            .Produces(StatusCodes.Status200OK, contentType: "application/prs.coveragejson+json")
            .Produces(StatusCodes.Status204NoContent)
            .Produces(StatusCodes.Status400BadRequest)
            .Produces(StatusCodes.Status404NotFound);
