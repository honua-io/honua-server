...
/// </summary>
internal sealed class ReadOnlyMySqlFeatureWriter : IFeatureWriter
{
    /// <inheritdoc />
    public Task<Feature> CreateAsync(int layerId, Feature feature, CancellationToken cancellationToken = default)
        => throw new NotSupportedException("MySQL/MariaDB provider is read-only. Feature creation is not supported.");

    /// <inheritdoc />
    public Task<Feature> UpdateAsync(int layerId, Feature feature, CancellationToken cancellationToken = default)
        => throw new NotSupportedException("MySQL/MariaDB provider is read-only. Feature updates are not supported.");

    /// <inheritdoc />
    public Task<bool> DeleteAsync(int layerId, long featureId, CancellationToken cancellationToken = default)
        => throw new NotSupportedException("MySQL/MariaDB provider is read-only. Feature deletion is not supported.");
    ...
    public Task<FeatureEditResult> ApplyEditsAsync(int layerId, FeatureEditBatch editBatch, CancellationToken cancellationToken = default)
        => throw new NotSupportedException("MySQL/MariaDB provider is read-only. Batch edits are not supported.");
}
