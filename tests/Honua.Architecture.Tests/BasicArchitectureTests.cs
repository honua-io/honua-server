...
public class BasicArchitectureTests
{
    ...
    public void ProjectFiles_ShouldHaveRequiredStructure()
    {
        ...
        Directory.Exists(Path.Combine(projectRoot, "tests", "Honua.TestKit")).Should().BeTrue($"Honua.TestKit project should exist in {projectRoot}");
        Directory.Exists(Path.Combine(projectRoot, "tests", "Honua.Server.Tests")).Should().BeTrue($"Honua.Server.Tests project should exist in {projectRoot}");
        Directory.Exists(Path.Combine(projectRoot, "tests", "Honua.Architecture.Tests")).Should().BeTrue($"Honua.Architecture.Tests project should exist in {projectRoot}");
    }
}
