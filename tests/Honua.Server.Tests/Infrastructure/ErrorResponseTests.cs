...
/// <summary>
/// Tests for Esri-compatible error response format
/// Ensures compliance with ArcGIS Server error response specifications
/// </summary>
public class ErrorResponseTests
{
    [Fact]
    public void ApiErrorResponse_ShouldSerializeToEsriCompatibleFormat()
    {
        // Arrange
    ...
    public void EsriErrorCodes_FromHttpStatusCode_ShouldReturnCorrectCode(int httpStatusCode, int expectedEsriCode)
    {
        // Act
        var actualEsriCode = EsriErrorCodes.FromHttpStatusCode(httpStatusCode);

        // Assert
        actualEsriCode.Should().Be(expectedEsriCode);
    }

    [Fact]
    public void EsriError_ShouldHaveRequiredProperties()
    {
        // Arrange & Act
        var esriError = new EsriError
        {
            Code = 404,
            Message = "Not found"
        };
        ...
        // Assert
        esriError.Code.Should().Be(404);
        esriError.Message.Should().Be("Not found");
        esriError.Details.Should().BeNull();
    }

    [Fact]
    public void ApiErrorResponse_ShouldMatchExactEsriFormat()
    {
        // Arrange
        var errorResponse = new ApiErrorResponse
        {
        ...
        // Assert - Verify exact JSON structure matches Esri specification
