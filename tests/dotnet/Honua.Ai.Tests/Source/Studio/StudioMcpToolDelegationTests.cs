...
public sealed class StudioMcpToolDelegationTests
{
    ...
    public async Task FullCompositionLifecycle_RoundTripsThroughTheRealLifecycleService()
    {
        ...
        // Every honua_studio_* write authorizes the StudioDraft grant family with
        // OperatorOperation.Create -- the canonical "studio-compose" write action
        // (#3412). Draft mutation is not modelled as a separate Update action, so
        // no tool on this path may ask for one.
