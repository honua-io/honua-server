...
public sealed class McpRestFeatureReadParityIntegrationTests : IAsyncLifetime
{
    ...
    public async Task FeatureRead_SamePrincipalOverRestAndMcp_ReturnsTheSameRecordSetAndNoDeniedLayerRows()
    {
        ...
        var count = await CallQueryFeaturesAsync(reader, AllowedLayerId, returnCountOnly: true);
        count.GetProperty("isError").GetBoolean().Should().BeFalse();
        count.GetProperty("structuredContent").GetProperty("count").GetInt64().Should().Be(5);
        ...
        (await ReadOverMcpAsync(restricted, DeniedLayerId)).Should().Equal(restrictedRows);
    }
