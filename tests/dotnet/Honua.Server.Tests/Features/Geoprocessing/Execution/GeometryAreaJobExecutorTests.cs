...
/// <summary>
/// Unit coverage of the slice-3 <c>geometry.area</c> executor. Pins the same
/// failure surfaces slice 1/2 introduced (unsupported process id, malformed
/// inputs, oversized artifact guardrail) plus the happy-path scalar measure
/// payload shape.
/// </summary>
public sealed class GeometryAreaJobExecutorTests
{
    ...
    public async Task ExecuteAsync_HappyPath_PublishesMeasureResult()
    {
        var executor = CreateExecutor();
        var context = Substitute.For<IJobExecutionContext>();
        context.OperationId.Returns("op-area");
        ...
        root.GetProperty("processId").GetString().Should().Be("geometry.area");
        root.GetProperty("measure").GetString().Should().Be("area");
        root.GetProperty("value").GetDouble().Should().BeApproximately(100.0, 1e-9);
        root.GetProperty("unit").GetString().Should().Be("input-crs-units-squared");
        root.GetProperty("inputSrid").GetInt32().Should().Be(4326);
        root.GetProperty("inputGeometryType").GetString().Should().Be("Polygon");
    }
    ...
    private static GeometryAreaJobExecutor CreateExecutor(long maxArtifactBytes = 50L * 1024L * 1024L)
    {
        ...
        return new GeometryAreaJobExecutor(monitor, NullLogger<GeometryAreaJobExecutor>.Instance);
    }
