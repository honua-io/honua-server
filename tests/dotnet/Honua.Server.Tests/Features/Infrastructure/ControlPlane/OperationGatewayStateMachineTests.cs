...
public sealed class OperationGatewayStateMachineTests
{
    ...
    [Fact]
    public async Task ApprovalReplay_RejectsScopeCeilingThatDoesNotPermitOperation()
    {
        var ladder = Substitute.For<IGuardrailLadder>();
        ladder.Resolve(OperationClass.Deploy).Returns(
            new GuardrailDecision(GuardrailTier.RequiresApproval, OperationClass.Deploy, HonuaEdition.Pro, "test"));
        ...
        var sut = BuildGateway(store: store, executor: executor, ladder: ladder);
        ...
        replayed!.Status.Should().Be(OperationProposalStatus.Failed);
        replayed.Plan.BlockingReasons.Should().ContainMatch("*exceeds the sealed OAuth scope authority*");
