...
public sealed class GeoservicesGeoportalImportIntegrationTests : IAsyncLifetime
{
    ...
    private static readonly GeoservicesImportServiceCase[] _serviceCases =
    [
    ...
    public async Task Import_GeoportalLayers_Completes()
    {
        foreach (var serviceCase in _serviceCases)
        {
            ...
            var response = await _client.PostAsJsonAsync("/api/v1/admin/import/geoservices/start", startRequest);
            ...
            ReadStatus(progress.GetProperty("status")).Should().Be(GeoservicesImportStatus.Completed);
        ...
        TimeSpan timeout)
    {
        ...
        {
            using var response = await client.GetAsync($"/api/v1/admin/import/geoservices/jobs/{jobId}");
            ...
            if (status == GeoservicesImportStatus.Completed)
            {
                return root;
            }

            if (status is GeoservicesImportStatus.Failed or GeoservicesImportStatus.Cancelled)
            {
                var error = root.TryGetProperty("errorMessage", out var errorElement)
                    ? errorElement.GetString()
                    : "Import failed.";
                throw new InvalidOperationException($"Geoservices import job {jobId} {status}: {error}");
            }
    ...
    private sealed record GeoservicesImportServiceCase(string ServiceUrl, int LayerId, string TablePrefix);
}
