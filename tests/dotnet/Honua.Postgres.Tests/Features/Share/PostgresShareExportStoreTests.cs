...
public sealed class PostgresShareExportStoreTests(PostgresFixture fixture)
{
    ...
    private async Task EnsureShareTablesAsync(string schema)
    {
        ...
        var migrationPath = Path.Join(root, "src", "Honua.Server", "Migrations", "037_CreateShareExportTraffic.sql");
    ...
    private static string FindRepoRoot()
    {
        ...
        while (current is not null)
        {
            ...
            if (File.Exists(Path.Join(current.FullName, "src", "Honua.Server", "Migrations", "037_CreateShareExportTraffic.sql")))
            {
