...
raster("statistics-masked.tif", [[[0, 1, 2], [3, 4, -9999]], [[-5, -9999, 5], [-9999, 15, 25]]])
masked = gdal.Open(str(root / "statistics-masked.tif"), gdal.GA_Update)
masked.CreateMaskBand(gdal.GMF_PER_DATASET)
masked.GetRasterBand(1).GetMaskBand().WriteArray(np.array([[0,255,255],[255,255,255]], dtype=np.uint8))
masked.Close()
