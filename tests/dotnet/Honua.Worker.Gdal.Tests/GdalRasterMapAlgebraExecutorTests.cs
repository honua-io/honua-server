...
public sealed class GdalRasterMapAlgebraExecutorTests
{
    ...
    public async Task MapAlgebra_TwoSources_ProjectsBandVariables_AndPublishesGeoTiff()
    {
        ...
        try
        {
            ...
            invocation.Arguments.Should().Contain("--calc=(lambda value: nan_to_num(value.astype(float64),nan=-9999,posinf=-9999,neginf=-9999) if issubdtype(value.dtype,inexact) else value)(asarray((A-B)/(A+B)))");
    ...
    public async Task MapAlgebra_LeadingUnaryMinus_PassesCalcAsSingleToken()
    {
        ...
        try
        {
            ...
            args.Should().Contain("--calc=(lambda value: nan_to_num(value.astype(float64),nan=-9999,posinf=-9999,neginf=-9999) if issubdtype(value.dtype,inexact) else value)(asarray(-A))");
