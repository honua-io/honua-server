...
public sealed class FeatureQueryBuilderProjectedPointTests
{
    ...
    public void BuildProjectedPointQuery_WithRasterGrid_UsesDistinctPointCellsAndLimit()
    {
        ...
        result.Sql.Should().Contain("SELECT AVG(x)::double precision AS x, AVG(y)::double precision AS y FROM snapped_points GROUP BY cell_x, cell_y ORDER BY cell_x, cell_y");
