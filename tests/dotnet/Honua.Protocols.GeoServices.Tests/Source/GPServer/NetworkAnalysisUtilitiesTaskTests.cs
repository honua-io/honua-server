...
public sealed class NetworkAnalysisUtilitiesTaskTests : IClassFixture<NAServerEndpointTestsFixture>
{
    ...
    public async Task ServiceInfo_KeepsTheUtilityTasksOutOfTheCallableTaskList()
    {
        // The task list is the callable-name set SOAP GetTaskNames must equal; the utility
        // tasks have no SOAP or job form, so they resolve by name without being listed.
        ...
        tasks.Should().NotContain(["GetTravelModes", "GetToolInfo"]);
