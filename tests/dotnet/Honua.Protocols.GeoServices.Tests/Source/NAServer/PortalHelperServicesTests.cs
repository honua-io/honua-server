...
public sealed class PortalHelperServicesTests : IClassFixture<NAServerEndpointTestsFixture>
{
    ...
    public async Task PortalsSelf_AdvertisesTheRoutingHelperServicesTheProviderSupports()
    {
        ...
        helpers.GetProperty("routingUtilities").GetProperty("url").GetString().Should().EndWith("/rest/services/NetworkAnalysisUtilities/GPServer");
        using var utilityRoot = await _fixture.Client.GetAsync(helpers.GetProperty("routingUtilities").GetProperty("url").GetString());
        utilityRoot.StatusCode.Should().Be(HttpStatusCode.OK);
        using var utilityMetadata = JsonDocument.Parse(await utilityRoot.Content.ReadAsStringAsync());
        utilityMetadata.RootElement.GetProperty("executionType").GetString().Should().Be("esriExecutionTypeSynchronous");
