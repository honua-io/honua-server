...
public sealed class PbfQueryFormatterTests
{
    ...
    [Fact]
    public void FormatAsPbf_TimestampValues_UseInt64EpochIncludingZero()
    {
        var layer = CreatePointLayer() with
        {
            SchemaFields = [new MetadataV2Field { Name = "instant", Type = MetadataV2FieldType.DateTime }]
        };
        ...
        {
            var feature = Feature.Create(1, null, ImmutableDictionary<string, object?>.Empty.Add("instant", value));
            var (payload, _) = _sut.FormatAsPbf(QueryResult<Feature>.Create(1, [feature]), layer,
                ...
                geometryPrecision: null, maxAllowableOffset: null, outFields: ["instant"]);
