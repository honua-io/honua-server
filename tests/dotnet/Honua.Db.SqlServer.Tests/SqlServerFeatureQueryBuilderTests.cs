...
public class SqlServerFeatureQueryBuilderTests
{
    ...
    public void BuildSelectQuery_TextSearch_BindsLiteralTermsInProviderDialect()
    {
        ...
        {
            TextSearch = new FeatureTextSearch(["name", "category"],
                [new FeatureSearchTerm[] { new(term, false), new("closed", true) }]),
            Limit = 2,
            Offset = 1
        };
        var result = SqlServerFeatureQueryBuilder.BuildSelectQuery(BuildMapping(), query, _attributeColumns);
        Assert.Contains("CHARINDEX(", result.Sql, StringComparison.Ordinal);
    ...
    public void BuildSelectQuery_EmptyOutFields_OmitsAttributeColumns()
    {
        ...
        var defaults = SqlServerFeatureQueryBuilder.BuildSelectQuery(mapping, new FeatureQuery(), _attributeColumns);
        ...
        var unset = SqlServerFeatureQueryBuilder.BuildSelectQuery(
