...
SCRIPT_PATH = (
    Path(__file__).resolve().parents[3] / "scripts" / "claude-architecture-review.py"
)
SPEC = importlib.util.spec_from_file_location("claude_architecture_review", SCRIPT_PATH)
assert SPEC is not None
assert SPEC.loader is not None
CLAUDE_ARCHITECTURE_REVIEW = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(CLAUDE_ARCHITECTURE_REVIEW)
...
def test_resolve_base_ref_prefers_origin_tracking_branch(monkeypatch) -> None:
    ...
    monkeypatch.setattr(CLAUDE_ARCHITECTURE_REVIEW.subprocess, "run", fake_run)

    assert CLAUDE_ARCHITECTURE_REVIEW.resolve_base_ref("trunk") == "origin/trunk"
...
def test_local_architecture_review_uses_resolved_base_ref(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        CLAUDE_ARCHITECTURE_REVIEW, "get_changed_files", fake_get_changed_files
    )
    monkeypatch.setattr(
        CLAUDE_ARCHITECTURE_REVIEW,
    ...
    results = CLAUDE_ARCHITECTURE_REVIEW.local_architecture_review()
