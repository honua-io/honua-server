...
writes a .cert.json envelope to ``ARCGIS_STUB_OUTPUT_DIR``.
...
def run() -> int:
    ...
    # Fill in the desktop-only IDs as 'skip' with a pending-runner note so
    # the matrix still has a 24-row envelope.
    ...
    # Any CORE_IDS still unhandled → not-applicable or skip per applicability.
    for cid in CORE_IDS:
        if cid in results:
            continue
    ...
    out_path = OUTPUT_DIR / f"{run_id}-arcgis-stub-featureserver.cert.json"
