...
- **API key**: `X-API-Key` header using `HONUA_ADMIN_PASSWORD` (supports `env:` references).
- **OIDC**: Bearer tokens when `Oidc:Enabled=true`; admin roles are configured via `Oidc:AdminRoles`.
...
| Admin | `/api/v1/admin/*` | Admin policy | Admin policy | API key or OIDC admin role required. |
...
- **Admin policy**: Uses `Admin`/`AdminPolicy` roles from API key or OIDC role claims.
