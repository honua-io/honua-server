...
- [ ] Verify the nightly `client-interop-nightly.yml` workflow passes with zero `pass`→`fail` regressions across the docker/client-compat lanes (`gdal`, `pyqgis`, `openlayers`, `cesium`, `arcgis-stub`); review the committed [`docs/gis/gap-report.md`](../gis/gap-report.md) for any new entries and confirm permanent gaps still match the documented rationale (Cesium vector-feature exclusions, ArcGIS Pro pending licensed runner)
