...
- **Tier dispatch (ADR-0037)**: each TestKit attribute already emits a `Tier` trait (`Fast` / `Integration` / `Slow`). New test attributes must emit exactly one tier from `Honua.TestKit.Constants.Tiers`. Tier assignment is additive — do not remove the existing `Category` trait.
- **Selective shard map**: when adding a `server-tests` shard, edit `.github/ci-shards.json` only — it is the single source of truth for both the shard's routing data (`paths`) and its matrix-runtime metadata (`shard_name`, `filter`, `artifact_suffix`, `log_name`, `timeout_minutes`, `test_timeout_minutes`, `max_cpu_count`, upload flags). The `targeted-shards` job in `ci.yml` projects the selected entries into the `server-tests` matrix at runtime via `strategy.matrix.include: fromJson(...)`, so unselected shards never instantiate a runner.
