...
| Azure | Webhook persists the event and enqueues a reconciliation job, ACKs in well under 10 s. The reconciler then implements the SaaS Fulfillment v2 contract — `Get Operation` to validate the payload (Microsoft requires this before acting), `Get Subscription` for the latest entitlement state when needed, POST to the hosted mint, apply via `FileLicenseStore`, and `PATCH operation status` (required for `ChangePlan`/`ChangeQuantity` so Microsoft does not roll the change back after 24 h). The customer-side webhook never blocks on any of those calls. |
