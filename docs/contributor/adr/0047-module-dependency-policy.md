...
| 5 | **Storage Providers** | `Honua.Postgres` (+ planned sub-assemblies `Honua.Postgres.{Migrations, Catalog, FeatureStore, Streaming, Outbox}`), `Honua.DuckDB`, `Honua.MySql`, `Honua.SqlServer`, `Honua.Oracle` | One assembly per backend. Implements Core abstractions over a specific provider SDK. Never references Hosting, Server, or another storage provider. | Provider SDK (`Npgsql`, `DuckDB.NET`, `MySqlConnector`, `Microsoft.Data.SqlClient`, `Oracle.ManagedDataAccess.Core`) + Abstractions/Core. |
...
| 6 | **Plugin SDK** | `Honua.Plugins.Abstractions` (public contract surface), `Honua.Plugins` (host runtime) | Enterprise plugin/extension SDK (#347, ADR-0024). The Abstractions assembly is the lean, AOT-safe contract surface third-party plugins reference (it depends on `Honua.Core.Abstractions` only, for the canonical `Feature` type); protocol edit handlers consume it via `IPluginEditPipeline` to fire plugin validators/hooks. The host runtime registers and gates plugins and depends on Core (license-entitlement gate + audit sink). Never references Server or a storage provider. | `Microsoft.Extensions.*` + Abstractions / Core. |
...
| Consumer ↓ \ Provider → | Abstr | Core | Geo | Gcod | Aws | Azure | Hosting | Jobs | Ai | Gproc | Io | Imp | Postgres | DuckDB | MySql | SqlServer | Oracle | Protocols.X | Server | SvcDef |
|--------------------------|:-----:|:----:|:---:|:----:|:---:|:-----:|:-------:|:----:|:--:|:-----:|:---:|:---:|:--------:|:------:|:-----:|:---------:|:------:|:-----------:|:------:|:------:|
| **Abstractions**         |       |      |     |      |     |       |         |      |    |       |       |       |          |        |       |           |        |             |        |        |
| **Core**                 |   ✓   |      |     |      |     |       |         |      |    |       |       |       |          |        |       |           |        |             |        |        |
| **Geometry**             |   ✓   |  ✓   |     |      |     |       |         |      |    |       |       |       |          |        |       |           |        |             |        |        |
...
- **Storage providers are siblings.** Postgres / DuckDB / MySql / SqlServer /
  Oracle never reference each other. The matrix has no row crossing them.
