...
The `targeted-shards` job runs a **parity check** (Python, stdlib only) that compares every `shard_name`/`filter` pair in `ci.yml::server-tests` against `.github/ci-shards.json` and fails the PR if either side has an extra or mismatched entry. Then it emits the `{run_all, shards[], reason}` JSON descriptor consumed by the `server-tests` matrix. The `server-tests` job's first step (`Check if shard is targeted`) parses the descriptor and decides whether `matrix.shard_name` is in the selected set; every subsequent step is gated on its `skipped` output so an unselected shard exits cleanly without checkout, build, or test work. The membership check lives at the step level because GitHub Actions does not expose the `matrix` context inside `jobs.<job_id>.if` (only `github`, `needs`, `vars`, and `inputs` are available there). On `push` to `trunk` and `workflow_dispatch` the descriptor is forced to `run_all: true`, so every shard runs.

## Test Tier Strategy

ADR-0035 defines a `Tier` xUnit trait — `Fast`, `Integration`, or `Slow` — that the existing TestKit attributes emit alongside the legacy `Category` trait. Tier assignment is **additive on the attribute**, so no test method needs to be re-tagged for the default mapping. See [ADR-0035: Unified CI Test Tier Strategy](adr/0035-unified-ci-test-tier-strategy.md) and [TestKit attributes](testkit.md#test-categories-and-tiers) for the full mapping.
...
The shard map has a single source of truth: `.github/ci-shards.json` mirrors the `shard_name` and `filter` values from `ci.yml::server-tests`, plus the `paths` prefixes that route a changed file to a shard. Adding or renaming a shard requires editing both files in the same PR — the parity check inside `targeted-shards` fails the run if they disagree. When a PR touches a source path under `unmapped_source_run_all_prefixes` (`src/Honua.Server/`, `src/Honua.DuckDB/`, `tests/dotnet/Honua.Server.Tests/`) that no shard claims, the script emits `{"run_all": true, "reason": "unmapped_source_change"}` so new feature directories run on a full matrix until a follow-up PR adds explicit shard routing.

`Tier=Slow` tests still respect their existing env-var skip logic (`HONUA_TEST_DB_URL`, `HONUA_TEST_S3_*`, `HONUA_TEST_AZURE_BLOB_*`, etc.). The nightly workflow asserts `HONUA_TEST_DB_URL` is set before the test step; the S3 and Azurite endpoints are owned by `EmulatorFixture` (a single Testcontainers-backed provisioning path) and the `[EmulatorTest]` attribute fills in defaults for any missing emulator env vars, so the workflow does not pre-provision LocalStack/Azurite via docker compose.
...
| `client-interop-nightly.yml` | Real-client interop matrix (Docker: gdal, pyqgis, openlayers, cesium, arcgis-stub) | Zero baseline `pass`→non-`pass` regressions vs `tests/baselines/client-compat/`; missing baseline envelopes, missing `expected-pairs.json` evidence, and new `fail` in unbaselined cases also fail the gate. Baseline-diff failure surfaces in `docs/gis/gap-report.md` |
