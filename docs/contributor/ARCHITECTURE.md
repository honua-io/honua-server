...
- **Tiles**: MVT + TileJSON.
- **Geoprocessing**: gRPC `ProcessService` — plan validation, dry-run estimation, async job lifecycle. GeoServices REST GPServer adapter — Esri-compatible `submitJob`, job status, per-parameter results, and cancellation over the canonical process runtime. Workspace lifecycle management — artifact storage, retention policies, quota evaluation, promotion from temporary to durable workspaces, and background cleanup.
