...
- **Read/query-only by design**: edits, statistics, native MVT/FlatGeobuf/Geobuf/GML output, streaming GeoJSON, and KNN/nearest-neighbor are reported as unsupported in `FeatureProviderCapabilities.ReadOnlyMySql` and throw `NotSupportedException` at the query builder boundary. Cross-SRID filters and output transforms are likewise rejected — there is no portable `ST_Transform`; callers must pre-project.
- **User-managed tables**: unlike PostGIS (internal `features` table discriminated by `layer_id`), MySQL targets user tables. `MySqlFeatureStore` resolves `LayerStorageMapping` via `ILayerCatalog` and registers a per-request `MySqlLayerMapping` in a scoped `MySqlLayerMappingRegistry` that the query builder reads to bind the qualified table, primary key column, geometry column, and SRID.
