...
- **Tiles**: MVT + TileJSON.
- **Geoprocessing**: gRPC `ProcessService` — plan validation, dry-run estimation, async job lifecycle. GeoServices REST GPServer adapter — job status polling and cancellation over the canonical process runtime; routes registered for service info (stub), task info, submitJob, execute, and per-parameter result retrieval (pending process catalog and execution engine). Workspace lifecycle management — artifact storage, retention policies, quota evaluation, promotion from temporary to durable workspaces, and background cleanup.
