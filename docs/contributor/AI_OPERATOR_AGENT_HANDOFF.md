...
If this handoff document and an issue body diverge, the issue body is
authoritative for that ticket's local scope and acceptance criteria, while this
handoff document is authoritative for cross-repo sequencing, load order, and
coordination rules.
...
live issue body includes additional ticket-specific read paths, agents should
take the union of the issue body and this section rather than choosing one over
the other.
...
- `honua-io/honua-server#728`
  - read `docs/developer/MCP_SERVER.md`, `docs/developer/AI_OPERATOR_CONTRACT.md`, the `geospatial-mcp` issue chain, and the `honua-sdk-js` MCP package at `mcp/README.md` and `mcp/src/index.ts`
...
- `honua-io/honua-server#738`
  - read `docs/developer/MCP_SERVER.md`, `docs/developer/AI_OPERATOR_CONTRACT.md`, `docs/developer/DETERMINISTIC_OPERATOR_WORKFLOW_RESULTS.md`, and the live issue bodies for `honua-io/geospatial-mcp#4`, `honua-io/honua-server#728`, `honua-io/honua-server#730`, `honua-io/honua-server#731`, and `honua-io/honua-server#732`
...
Issue bodies may use local repo phase labels such as `H*`, `J*`, `S*`, and
`D*`. Treat those as repo-local planning markers only. The wave ordering in
this handoff is the authoritative cross-repo execution sequence.
...
dependency as satisfied until `#681` closes or the live issue body is updated
...
`honua-io/honua-server#722`. OGC API Processes, GeoServices `GPServer`, and
broader built-in GP family expansion remain important backlog, but they are not
prerequisites for starting the operator-plane implementation. For meaningful
...
- `honua-io/honua-server#727` may not close until the external durable-worker
  substrate in `honua-io/honua-server#681` is closed or the live issue body is
...
- read the issue body and any linked dependency issues
...
- the issue and handoff both identify the baseline code to study first when
  reuse is expected
