...
| WCS 2.0.1 / WCS 2.0 ETS | Service-level CITE | `ogccite/ets-wcs20:1.22-teamengine-6.0.0-RC2`; generated TeamEngine params use `ets-wcs20-1.22`; default profile `core`; optional `crs`, `extensions`, `full` | `scripts/conformance/cite/run-cite-wcs20-tests.sh`; `.github/workflows/cite-wcs20-conformance.yml` | PR #993 local evidence on branch `codex/cite-nightly-fix`; summary path `cite-wcs20-results/cite-wcs20-summary.md` | PASS: 82 total, 82 passed, 0 failed, 0 skipped, 0 CantTell | Good internal service-level evidence for the WCS 2.0 core/KVP profile. Still not certification because no OGC submission/review and the retained artifact is not durable release evidence. |
...
- WCS 2.0.1 has retained Actions evidence for the selected profile.
