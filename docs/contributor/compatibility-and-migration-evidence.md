...
> GeoServer apply-plan/apply-execution evidence, can idempotently publish
> PostGIS-backed GeoServer layers whose source tables already exist in the
> target Honua database, emits classic OGC WFS/WMS/WMTS migration planning
> artifacts, and produces migration evidence artifacts for review
> before cutover.
...
| GeoServer REST | Automated inventory, dry-run validation, deterministic apply-plan generation, and bounded PostGIS-backed catalog apply | [Import and Migration Capability Evidence](import-capability-evidence.md); [GeoServer to Honua Migration Guide](../gis/tutorials/geoserver-migration-guide.md); `POST /api/v1/admin/import/geoserver/start` can emit `honua.migration.apply-plan` and `honua.migration.apply-execution` artifacts | Data copy, layer groups, WMS/WFS/WMTS exposure changes, bulk style persistence, and release-gated fixture parity remain open under applied GeoServer migration: [honua-server#1015](https://github.com/honua-io/honua-server/issues/1015). |
...
| Migration cost/performance evidence | Release-gated `honua.migration.performance-evidence` artifact is published by the [Release Migration Performance Evidence workflow](../../.github/workflows/release-migration-performance.yml); first GeoServer-small baseline (`geoserver-small-v1`) is seeded, broader fixture coverage is staged | [Migration Performance Evidence](../evidence/migration-performance-evidence.md) — schema, fingerprint, redaction posture, and the latest passing run | Seed additional baselines (ArcGIS, OGC Features, OGC map/tile metadata, coverage, plus GeoServer Medium/Large) and link a passing release-gated run before broad "minimal-cost migration" wording: [honua-server#1033](https://github.com/honua-io/honua-server/issues/1033). |
...
| Python GP / ArcPy process migration | Server first-slice evidence contracts now define concrete vector process ids, result route expectations, heavyweight/destructive classification, and fixture artifact shape; existing ArcPy script/toolbox translation and populated parity runs are not complete | [MVP Compatibility and Limitations](../gis/MVP_COMPATIBILITY_CONTRACT.md); [Geoprocess Framework Analysis](../gis/geoprocess-framework-analysis.md); [Process Migration Evidence](process-migration-evidence.md); Python SDK OGC Processes client and migration artifact models | Add ArcPy/Python GP scan, translate, execute, parity, and readiness evidence: [honua-sdk-python#59](https://github.com/honua-io/honua-sdk-python/issues/59). Populate and link passing process evidence runs through [honua-server#1031](https://github.com/honua-io/honua-server/issues/1031), [honua-server#1024](https://github.com/honua-io/honua-server/issues/1024), and [honua-server#1018](https://github.com/honua-io/honua-server/issues/1018). |
