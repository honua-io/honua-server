...
| Cache keys | Vary by tenant, caller authorization context, credential/token transport when a shared cache helper supports it, scene id, host/scheme, export schema version, access policy, resolved scene revision or source ETag inputs, and `Accept`. Protected output must be private or no-store according to the scene policy; use no-store or bypass shared caching unless the cache path varies by the same auth and tenant inputs. |
