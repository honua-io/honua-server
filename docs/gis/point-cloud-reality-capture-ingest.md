...
- Operators run the conversion outside Honua using existing open-source
  toolchains (PDAL `tile`, `potree-converter`, or photogrammetry exports
  from Metashape / RealityCapture). Zero new server dependencies, zero
  cloud spend, zero new endpoints.
- The
...
| **LAS / LAZ** | Point cloud | Medium — needs a CPU PDAL conversion job to produce `.pnts` 3D Tiles or a COPC | Source artifact + derived tiles | CUDA-accelerated PDAL filters | Industry-standard scanner output; large files; conversion is a follow-up executor. |
...
2. Operator runs an external open-source pipeline:
   ...
   - **Point cloud**: PDAL pipeline (`pdal pipeline`) or `potree-converter`
     emits `.pnts` 3D Tiles, or a COPC file when serving the source point
     cloud is preferred over tiling.
...
The local-storage provider (`LocalFileStorage`) backs the asset root; an S3
or Azure deployment uses the same `ICloudFileStorage` abstraction without
changing the demo flow.
...
| Storage layout | Source files under `captures/{sessionId}/source/`, derived tiles under `captures/{sessionId}/tiles/`, both via `ICloudFileStorage`. `LocalFileStorage` covers the local demo with no S3/Azure dependency. |
...
| Job kind | A new `ExecutionJobKind.PointCloudIngest` value extends the existing enum (`Geoprocessing`, `ExtractTransformLoad`, `TileCache` today) for the CPU PDAL path. A separate `PointCloudIngestGpu` is added when the GPU/Batch slice is implemented. |
| Executor pattern | A new `IJobExecutor` implementation registered in DI, parallel to the placeholder slot the v1 pipeline already exposes. Conversion shells out to PDAL via `Process.Start` to keep the server image slim and AOT-safe. |
...
| --- | --- | --- | --- |
| Point-cloud tiling and reprojection | PDAL with CUDA filters | Phase 2 (CPU first, GPU follow-up) | New `IJobExecutor` + `ExecutionJobKind.PointCloudIngest`. |
...
- Derived tiles: `captures/{sessionId}/tiles/`, registered as a
  `SceneDataset` so the existing serving path applies access policy and
  cache headers.
...
- **PDAL shell-out vs managed library.** `Process.Start` keeps the server
  image AOT-safe and avoids a heavyweight native dependency, at the cost of
  requiring PDAL in the Phase 2 executor's deployment environment. The
  existing AWS Batch / Azure Batch container model is the natural home —
  the server stays slim.
...
| 5 | `feat(jobs): point-cloud ingest CPU executor (PDAL shell-out)` | `honua-server` | New `IJobExecutor` for `ExecutionJobKind.PointCloudIngest`. Shell-out to PDAL via `Process.Start`. Writes outputs to `ICloudFileStorage`, then registers via `ISceneRegistrationService`. Progress to `IExecutionLogStore`. Integration test with a small synthetic LAS fixture. | Children #1, #2, #3. | Proposed; not filed by this spike. |
| 6 | `feat(jobs): GPU-capable Batch submission (`PointCloudIngestGpu`)` | `honua-server` | Add `GpuInstanceType` (or `GpuCount`) to `ExecutionJobSpec`; wire `AwsBatchJobSubmission.GpuCount` and the Azure equivalent through the spec; new `ExecutionJobKind.PointCloudIngestGpu`; container image selection for CUDA-enabled PDAL. | Child #5. | Proposed; not filed by this spike. |
