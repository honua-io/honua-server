...
| Field | Type | Description |
|---|---|---|
| `anchor` | string \| null | The newest non-future `run_date` across all committed `tests/baselines/client-compat/**/*.cert.json` envelopes (`null` when no envelope has a usable, non-future timestamp). Future-dated envelopes never become the anchor — they would self-report as fresh while making legitimate evidence look stale — and instead fail closed as `stale`. All `interop[].freshness.ageDays` values are measured against this anchor. |
