...
| Spatial analytics — clusters (extension, Pro) | N/A | POST | Implemented | `POST /ogc/features/collections/{collectionId}/clusters` | Honua extension. POST-only mirror of `queryClusters` from FeatureServer; DBSCAN or K-Means clustering with the same request body, response payload, and edition gate (`analytics.clustering`, ADR-0024). |
