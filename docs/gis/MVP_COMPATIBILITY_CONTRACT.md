...
| GeoServices REST GPServer | Supported with partial parity | PrintingTools (Export Web Map, Layout Templates); generic GPServer adapter: service info (stub metadata), job status, job cancel over canonical process runtime | Synchronous execute returns 501 (pending #721); task info and submitJob return 501 pending per-task projection of the built-in `IProcessCatalog` (19 seeded processes across `geometry.*`, `analytics.*`, `generalization.*`, `data-management.*`) into the GPServer surface; per-parameter result retrieval route registered, output retrieval pending execution engine; GP env controls (`env:*`, `context`) rejected | [Geoprocess Framework Analysis](geoprocess-framework-analysis.md) |
...
| OGC API Maps | Supported | Conformance, dataset map, collection map, map tiles | 32/32 conformance tests passing | Styled-map conformance class not claimed; `/ogc/maps/collections/{id}/styles/{styleId}/map` is not implemented |
| OGC API Processes | Supported (async-only) | Landing/conformance, process list/describe, async execution, job list/status/dismiss; plan submissions are validated against the built-in `IProcessCatalog` (19 seeded processes across `geometry.*`, `analytics.*`, `generalization.*`, `data-management.*`) at the adapter boundary; destructive `data-management.*` plans route through the operator approval gate before execution | V1: async-only (sync returns 501), single canonical process projection (`honua-geoprocessing`) pending per-process OGC surface projection, results endpoint still stubbed (`404` non-terminal/successful, `500` failed, `410` dismissed), `limit`-only job list filter, Redis-backed job store required for execution and job routes | [OGC API Processes Coverage](specifications/ogc-api-processes-coverage.md) |
