...
| OGC API Processes | Supported (async-only) | Landing/conformance, process list/describe, async execution, job status/results/dismiss | V1: async-only (sync returns 501), single canonical process, document-mode by-value results, `limit`-only job list filter | [OGC API Processes Coverage](specifications/ogc-api-processes-coverage.md) |
