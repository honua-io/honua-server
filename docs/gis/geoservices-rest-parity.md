...
| GPServer | Supported with partial parity | PrintingTools GPServer surface (Export Web Map Task, Get Layout Templates Info Task). Generic GPServer adapter (#723): service info, task info, submitJob, job status, job result (route registered; actual output retrieval pending execution-engine/result-storage support), cancel job over the canonical process runtime. | Synchronous execute (501 pending #721 ExecutePlan). Task/service discovery returns stub metadata until a formal process catalog is available. | [Geoprocess Framework Analysis](geoprocess-framework-analysis.md) |
