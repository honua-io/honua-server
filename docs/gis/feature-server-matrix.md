...
These four layer-level operations are Honua extensions to the GeoServices REST surface — they have no direct Esri equivalent, since Esri sells the comparable functionality through the separately licensed Spatial Analyst, Network Analyst, and GeoAnalytics Server products. They are gated to Pro under [ADR-0024](../contributor/adr/0024-open-core-edition-model.md) (`analytics.clustering`, `analytics.spatial-join`, `analytics.buffer-aggregate`, `analytics.density`) and return HTTP 402 with code `edition.upgrade_required` when invoked on Community.

All four endpoints honor the shared filter AST (`where`, `objectIds`, spatial filter, temporal filter), use the standard GeoServices `outStatistics` payload, and are bounded by `Limits.Analytics.MaxInputFeatures` via a SQL `LIMIT n+1` overflow probe. Responses are GeoJSON FeatureCollections with a `metadata` envelope (`operation`, `inputTruncated`, `resultTruncated`, `maxInputFeatures`, `maxOutputRows`). Form-encoded request bodies are accepted in addition to JSON, matching the rest of the GeoServices REST handlers.

| Honua extension | Methods | Honua endpoint(s) | Notes |
| --- | --- | --- | --- |
| Query Clusters | POST | `POST /rest/services/{serviceId}/FeatureServer/{layerId}/queryClusters` | DBSCAN (`algorithm=dbscan`, `eps`, `minPoints`) or K-Means (`algorithm=kmeans`, `k`). `eps` is meters; `k` is capped by `Limits.Analytics.MaxKMeansK`. `returnHullPerCluster=true` returns one feature per cluster with the convex hull and any `outStatistics`; the default returns one row per source feature with its assigned `clusterId`. Hull-mode output is bounded by `Limits.Analytics.MaxClusters`. |
| Spatial Join | POST | `POST /rest/services/{serviceId}/FeatureServer/{layerId}/spatialJoin` | Joins the target layer against `joinLayerId` (must differ) using `predicate=intersects|contains|within|dwithin`. `dwithin` requires a positive `distance` in meters, capped by `Limits.Analytics.MaxDWithinDistanceMeters`. `carryFields` is a comma-separated list of join-layer columns; `outStatistics` aggregates over matched rows. The join layer is resolved through the same `IResourceValidator` access policy as the target so its parent service must also be enabled and readable. |
| Query Buffer Aggregate | POST | `POST /rest/services/{serviceId}/FeatureServer/{layerId}/queryBufferAggregate` | Buffers each input feature by `distance` (in `unit`: `meters`/`kilometers`/`feet`/`miles`, aliases `m`/`km`/`ft`/`mi`). The cap (`Limits.Analytics.MaxBufferDistanceMeters`) is enforced in meters after unit conversion so it cannot be bypassed by switching units. `dissolve=true` (default) unions overlapping buffers per group via `ST_Union`; `groupByFields` and `outStatistics` follow the standard GeoServices statistics shape. `outStatistics` requires `dissolve=true` — per-feature output (`dissolve=false`) cannot carry aggregate columns and the server returns HTTP 400 if both are supplied. |
| Query Density | POST | `POST /rest/services/{serviceId}/FeatureServer/{layerId}/queryDensity` | Hex (`mode=hex`/`hexgrid`/`hex-grid`) or square grid (`mode=square`/`squaregrid`/`square-grid`) binning over Web Mercator. `cellSize` is meters and is clamped to `[Limits.Analytics.MinDensityCellSizeMeters, Limits.Analytics.MaxDensityCellSizeMeters]`. Optional `weightField` sums a numeric column instead of feature counts; output is bounded by `Limits.Analytics.MaxDensityCells`. |
...
- Integration tests: [FeatureServerQueryParameterTests](../../tests/Honua.Server.Tests/Features/FeatureServer/FeatureServerQueryParameterTests.cs), [FeatureServerReplicationTests](../../tests/Honua.Server.Tests/Features/FeatureServer/FeatureServerReplicationTests.cs), [FeatureServerMaintenanceTests](../../tests/Honua.Server.Tests/Features/FeatureServer/FeatureServerMaintenanceTests.cs), [FeatureServerExceptionMappingTests](../../tests/Honua.Server.Tests/Features/FeatureServer/FeatureServerExceptionMappingTests.cs), [SpatialAnalyticsRestTests](../../tests/Honua.Server.Tests/Features/SpatialAnalytics/SpatialAnalyticsRestTests.cs), [SpatialAnalyticsOgcTests](../../tests/Honua.Server.Tests/Features/SpatialAnalytics/SpatialAnalyticsOgcTests.cs), [SpatialAnalyticsEditionGateTests](../../tests/Honua.Server.Tests/Features/SpatialAnalytics/SpatialAnalyticsEditionGateTests.cs)
