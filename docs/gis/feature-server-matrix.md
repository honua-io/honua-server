...
Responses are always `application/geo+json` FeatureCollections, even on the FeatureServer REST routes, and analytics geometries are normalized to WGS 84 / EPSG:4326 for GeoJSON compatibility. The `metadata` envelope always includes `operation`, `inputTruncated`, `resultTruncated`, `maxInputFeatures`, and `maxOutputRows`; `maxOutputRows` is populated for cluster hull mode and density, and `null` for per-feature clusters, spatial join, and buffer aggregate. Form-encoded request bodies are accepted in addition to JSON, matching the rest of the GeoServices REST handlers. Unsupported POST content types return HTTP 415 Unsupported Media Type, and the legacy `f` parameter does not change the analytics response media type.
