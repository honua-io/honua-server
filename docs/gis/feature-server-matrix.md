...
| Create Replica | `/rest/services/{serviceName}/FeatureServer/createReplica` | POST | Implemented | `POST /rest/services/{serviceId}/FeatureServer/createReplica` | Creates a named replica for offline synchronization. Parameters: `replicaName`, `layers`, `syncModel`. Replica state is persisted to Postgres and write-through cached; operator metadata records the authenticated owner and request `User-Agent` as `deviceClient` when present. |
| Extract Changes | `/rest/services/{serviceName}/FeatureServer/extractChanges` | POST | Implemented | `POST /rest/services/{serviceId}/FeatureServer/extractChanges` | Returns adds/updates/deletes since the replica's last synchronization generation. Replicas with `LastSyncGeneration == 0` use the backward-compatible initial extract path that reports all accessible features as adds. |
| Synchronize Replica | `/rest/services/{serviceName}/FeatureServer/synchronizeReplica` | POST | Implemented | `POST /rest/services/{serviceId}/FeatureServer/synchronizeReplica` | Synchronizes a replica. Supports `download`, `upload`, and `bidirectional` sync directions. Upload/bidirectional edits are partitioned: non-conflicting edits continue through `applyEdits`, while edits whose object id changed on the server since the replica base generation create durable conflict records and add `conflictCount`, `conflictIds`, and `syncOpId` to an otherwise successful response. |
