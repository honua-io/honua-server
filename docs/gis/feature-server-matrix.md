...
These four layer-level operations are Honua extensions to the GeoServices REST surface — they have no direct Esri equivalent, since Esri sells the comparable functionality through the separately licensed Spatial Analyst, Network Analyst, and GeoAnalytics Server products. They are gated by active entitlement keys under [ADR-0024](../contributor/adr/0024-open-core-edition-model.md) (`analytics.clustering`, `analytics.spatial-join`, `analytics.buffer-aggregate`, `analytics.density`) and return HTTP 402 Payment Required with the GeoServices error envelope (`error.code=402`, `message="Payment Required"`, details containing the inactive entitlement key and upgrade message) when the required entitlement is inactive.
