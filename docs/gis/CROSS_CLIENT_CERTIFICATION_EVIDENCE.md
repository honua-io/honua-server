...
```json
{
  ...
  "protocol": "<featureserver|mapserver|ogc-features|ogc-maps|ogc-tiles|3d-tiles|odata|mvt|wfs|wms|wmts|admin-api>",
...
| `protocol` | string | Yes | One of: `featureserver`, `mapserver`, `ogc-features`, `ogc-maps`, `ogc-tiles`, `3d-tiles`, `odata`, `mvt`, `wfs`, `wms`, `wmts`, `admin-api`. The `ogc-maps` and `ogc-tiles` values are emitted by the Cesium imagery lane (and by future OGC API Maps / Tiles producers) and align with the protocol abbreviations in [`CROSS_CLIENT_CERTIFICATION_MATRIX.md`](CROSS_CLIENT_CERTIFICATION_MATRIX.md#protocol-abbreviations). The `3d-tiles` value is emitted by the Cesium 3D Tiles scene smoke (`tests/js-browser/cesium/3d-tiles-scene.spec.ts`, ticket #838) for Honua's hosted OGC 3D Tiles surface (`/scenes/{sceneId}/`); only `CERT-CONN-01`, `CERT-AUTH-01`, `CERT-DISC-01`, `CERT-ERRH-01`, and `CERT-RNDR-01` are applicable to a `Cesium3DTileset` scene primitive — the remaining 24-ID common-core slots are `not-applicable`. |
...
docker/client-compat/output/
  cesium/        <run-id>-js-cesium-{wms,wmts,ogc-tiles,ogc-maps,3d-tiles}.cert.json
...
| Playwright CesiumJS suite | JS — Cesium (`js-cesium`) | Automated: `tests/js-browser/cesium/` exercises CesiumJS imagery providers (WMS, WMTS, OGC API Tiles, OGC API Maps) under `docker/client-compat/cesium/` in `client-interop-nightly.yml` and the `Cesium3DTileset` scene primitive (hosted OGC 3D Tiles, `/scenes/{sceneId}/`) under `tests/js-browser/cesium/3d-tiles-scene.spec.ts`; emits one envelope per protocol (`<run-id>-js-cesium-{wms,wmts,ogc-tiles,ogc-maps,3d-tiles}.cert.json`). Vector-feature CERT-\* IDs and the visual / style slice IDs are recorded as `not-applicable` because Cesium imagery providers consume server-rendered raster output and 3D Tiles scene primitives consume binary tile payloads, not per-feature drawing info. The 3D Tiles smoke is also the merge-blocking `cesium-3d-tiles-smoke` PR job in `ci.yml` (scoped to `--grep "3D Tiles"`); see [`tests/js-browser/cesium/README.md`](../../tests/js-browser/cesium/README.md) for the local command and required `Scenes:Datasets` / `Cors:AllowedOrigins` configuration. |
...
| 1.0.24 | 2026-05-01 | Add `3d-tiles` to the allowed `protocol` values for the Cesium 3D Tiles scene smoke (`tests/js-browser/cesium/3d-tiles-scene.spec.ts`); document its applicable CERT-\* subset (`CERT-CONN-01`, `CERT-AUTH-01`, `CERT-DISC-01`, `CERT-ERRH-01`, `CERT-RNDR-01` — remaining 24-ID common-core slots are `not-applicable` for a `Cesium3DTileset` scene primitive); extend the cesium nightly output mapping and Integration Mapping row to include the new `3d-tiles` envelope and the merge-blocking `cesium-3d-tiles-smoke` PR job (#838) |
