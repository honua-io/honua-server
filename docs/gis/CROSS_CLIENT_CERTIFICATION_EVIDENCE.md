...
The JS lane currently covers FeatureServer only (via Vitest). The Esri Leaflet sub-lane adds Playwright-generated `*-js-featureserver.cert.json` and `*-js-mapserver.cert.json` evidence (written to `tests/js-browser/evidence/`, not this curated directory). Additional JS evidence files (e.g., `*-js-ogc-features.cert.json`) will appear once Vitest OGC suites are added.
