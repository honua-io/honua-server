...
- `Cache-Control` is access-policy-aware: public scenes return `public, max-age=…` so CDNs/proxies can share-cache; protected scenes return `private, max-age=…`; bearer/API-key responses vary by `Authorization`, `?token=` responses vary by URL, and `X-Honua-Token` responses vary by that header so caches do not bypass the per-request access policy.
