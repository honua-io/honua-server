...
The deeper related-record review found concrete inherited behavior, independently of the empty fixture. QGIS AFS does not override the [base relation-discovery method](https://github.com/qgis/QGIS/blob/1a4cda5f2620e7374e5926fc955a7d2d06493e15/src/core/vector/qgsvectordataprovider.cpp#L1053), which returns an empty list. Both GDAL ESRIJSON datasource paths, including its paging wrapper, inherit the [base relationship methods](https://github.com/OSGeo/gdal/blob/c8b4c45fca87d3e6fbf80e7a7898b8a661ad0edc/gcore/gdaldataset.cpp#L10498), which return no names or relationship object. A positive fixture cannot enable these discovery APIs. This source finding does not rule out every possible alternative native entrypoint. Likewise, generic attachment-image downloading would only prove a display subcase, not attachment discovery or CRUD; ordinary QGIS offline copying/edit commits do not prove replica synchronization. The six exclusions remain provisional.

A separate server implementation limit applies to replica delivery: [the validated source](https://github.com/honua-io/honua-server/blob/8b7aea9f6c73504d968560227926e3b8b4c5ddd0/src/Honua.Protocols.GeoServices/FeatureServer/FeatureServerRequestHandlers.ReplicaDelivery.cs#L60) accepts Esri JSON, rejects SQLite/FileGDB and other file output, and rejects `returnAttachments=true` (tracked by #4405). Async replica generation is also explicitly rejected. These are parameter-level implementation limits; enabling an environment variable, licensing an extension or installing raster support does not enable them. They do not imply that JSON replica synchronization is absent, and no existing coverage cell is removed on this basis.
...
A later owned-branch REST/SQL control on the same `8b7` runtime identifies two further resolver defects. A plain branch name returned by create returns HTTP 200 with error code 500, while its GUID and owner-qualified lookup return the edited value 8124; correlated PostgreSQL error 42P08 identifies the optional owner parameter sent as untyped NULL. A branch created with an owner-qualified name returns that name but exact-name lookup returns HTTP 200 with error code 404, while its GUID returns 8125; the resolver splits the name before trying the stored identity. Both owned branches and deltas were removed, with full DEFAULT row hashes and pre-existing version identities unchanged. These are server diagnostics awaiting the canonical resolver repair, with no native-client pass or retroactive change to historical query receipts.
