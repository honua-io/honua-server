...
The **`#320` Run / Artifact Link** column should point to the uploaded smoke-artifact root (`artifacts/client-compat/<service>-<timestamp>/`) or the corresponding workflow artifact URL. Evidence files should point to the final per-client/protocol `.cert.json` produced during certification follow-through. The workflow does not emit those files automatically. See the [Cross-Client Certification Evidence Specification](CROSS_CLIENT_CERTIFICATION_EVIDENCE.md) for the envelope format.
