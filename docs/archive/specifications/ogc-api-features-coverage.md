...
| Spatial analytics — clusters (extension, Pro catalog tier) | N/A | POST | Implemented | `POST /ogc/features/collections/{collectionId}/clusters` | Honua extension. POST-only mirror of `queryClusters` from FeatureServer; DBSCAN or K-Means clustering with the same request body, response payload, and entitlement gate (`analytics.clustering`, ADR-0024). |
| Spatial analytics — spatial join (extension, Pro) | N/A | POST | Implemented | `POST /ogc/features/collections/{collectionId}/spatial-join` | Honua extension. POST-only mirror of `spatialJoin`; joins the source collection against `joinLayerId` using `intersects`/`contains`/`within`/`dwithin`. The join layer is resolved through the same `IResourceValidator` access policy as the source so its parent service must also be enabled and readable (`analytics.spatial-join`, ADR-0024). |
| Spatial analytics — buffer aggregate (extension, Pro) | N/A | POST | Implemented | `POST /ogc/features/collections/{collectionId}/buffer-aggregate` | Honua extension. POST-only mirror of `queryBufferAggregate`; buffers features by `distance` in `unit` (meters/kilometers/feet/miles), with `dissolve` and `groupByFields`. The buffer cap is enforced in meters after unit conversion (`analytics.buffer-aggregate`, ADR-0024). |
| Spatial analytics — density (extension, Pro) | N/A | POST | Implemented | `POST /ogc/features/collections/{collectionId}/density` | Honua extension. POST-only mirror of `queryDensity`; hex or square grid binning at `cellSize` meters with optional `weightField` (`analytics.density`, ADR-0024). |

The four spatial analytics extensions are POST-only Pro catalog-tier routes (ADR-0024) and return HTTP 402 Payment Required with a problem-details response when the required entitlement is inactive. The `detail` includes the inactive entitlement key and upgrade message. On deployments without a PostGIS-backed analytics reader (for example the DuckDB read-only provider), the same routes return HTTP 501 Not Implemented with a `StandardErrorResponse` body — see the [DuckDB provider](../../configuration/data-sources/duckdb.md#write-operations-return-errors) doc for the full envelope. They share their handler implementations with the GeoServices REST surface — see the [FeatureServer matrix](../../compatibility/feature-server-matrix.md#honua-spatial-analytics-extensions-pro-catalog-tier) for parameter semantics, limits, and response shape (`numberReturned`, `metadata.operation`, `metadata.inputTruncated`, `metadata.resultTruncated`, `metadata.maxInputFeatures`, `metadata.maxOutputRows`). Per-feature cluster and spatial-join mirrors preserve `properties.objectId` plus nested `properties.attributes`; aggregate operations expose operation-specific summary fields such as `featureCount`, `cellId`, and optional `weight`.
