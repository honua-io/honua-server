...
Geoprocess steps are validated against the built-in `IProcessCatalog` (34 seeded processes across `geometry.*`, `analytics.*`, `surface.*`, `raster.*`, `conversion.*`, `generalization.*`, and `data-management.*` as of this document's writing — archived/point-in-time; the catalog has since grown to 96 processes, see [the current geoprocessing operations reference](../../reference/geoprocessing-operations.md)) before a job is created. `surface.*` and `raster.*` process declarations are catalog- and validation-only at the adapter boundary today; `ISurfaceAnalysisService` and `IRasterStore.ComputeZonalStatisticsAsync` are the PostGIS-backed execution primitives those families will dispatch into when the handler/executor wiring (optionally routed over the #727 cloud executor adapters) lands in a follow-on ticket. The adapter surfaces the following structured violation codes:
...
Workspace and retention configuration is shared with the canonical geoprocessing runtime under `Geoprocessing:Workspace`. See [Operations Guide](../../../guides/deploy/operations.md) for workspace lifecycle settings.

## V1 Limitations

- **Async-only**: synchronous execution returns `501 Not Implemented` when the `Prefer: respond-async` header is absent.
- **Single process projection**: the OGC adapter lists one canonical process (`honua-geoprocessing`) even though the internal catalog now enumerates 34 built-in processes across seven families (`geometry.*`, `analytics.*`, `surface.*`, `raster.*`, `conversion.*`, `generalization.*`, `data-management.*`) as of this document's writing — archived/point-in-time; the catalog has since grown to 96 processes across 14 families, see [the current geoprocessing operations reference](../../reference/geoprocessing-operations.md). Per-process projection into `/processes` and `/processes/{id}` is follow-on adapter work; executions dispatch through the canonical process and are validated against the built-in catalog at the adapter boundary.
- **Heavyweight surface/raster execution**: `surface.*` and `raster.*` plan steps are declared in the catalog for discovery and plan validation today; handler/executor wiring that would dispatch them into `ISurfaceAnalysisService` / `IRasterStore.ComputeZonalStatisticsAsync` (optionally routed through the #727 cloud executor adapters) on the canonical worker boundary is follow-on work. The OGC adapter does not short-circuit that future path.
...
- [Geoprocess Framework Analysis](../../../guides/query-analyze/run-geoprocessing.md) — cross-protocol comparison (GPServer, OGC API Processes, GeoServer WPS)
- [ADR-0029: Geoprocess Canonical Model Mappings](../../../internal/contributor/adr/0029-geoprocess-canonical-model-mappings.md) — adapter contract and lifecycle state mapping
- [Geospatial APIs Overview](../../../concepts/protocols.md)
