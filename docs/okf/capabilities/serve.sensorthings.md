---
type: capability
title: "OGC SensorThings API"
description: "Query sensor observation data through OGC SensorThings API v1.1."
resource: "honua://capability/serve.sensorthings"
tags: [capability, serve, community]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# OGC SensorThings API

Query sensor observation data through OGC SensorThings API v1.1.

| | |
| --- | --- |
| Capability key | `serve.sensorthings` |
| Category | Serve |
| Edition | Community |
| Surface maturity | 23 preview |
| Registry entries | 23 |
| Proving tests | 98 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/serve.sensorthings"` to the page that documents it.
