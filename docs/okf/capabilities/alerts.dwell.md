---
type: capability
title: "Dwell Trigger"
description: "Trigger alerts when features remain inside a zone for a configured duration."
resource: "honua://capability/alerts.dwell"
tags: [capability, alerts, enterprise]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# Dwell Trigger

Trigger alerts when features remain inside a zone for a configured duration.

| | |
| --- | --- |
| Capability key | `alerts.dwell` |
| Category | Alerts |
| Edition | Enterprise |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/alerts.dwell"` to the page that documents it.
