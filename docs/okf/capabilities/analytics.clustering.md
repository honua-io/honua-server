---
type: capability
title: "Spatial Clustering"
description: "Server-side DBSCAN and K-Means clustering with optional cluster hulls."
resource: "honua://capability/analytics.clustering"
tags: [capability, analytics, pro]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# Spatial Clustering

Server-side DBSCAN and K-Means clustering with optional cluster hulls.

| | |
| --- | --- |
| Capability key | `analytics.clustering` |
| Category | Analytics |
| Edition | Pro |
| Surface maturity | 1 implemented |
| Registry entries | 1 |
| Proving tests | 16 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/analytics.clustering"` to the page that documents it.
