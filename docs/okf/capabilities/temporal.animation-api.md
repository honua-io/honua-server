---
type: capability
title: "Animation API Contract"
description: "Capability flag for SDK/admin TimeSlider and playback integration."
resource: "honua://capability/temporal.animation-api"
tags: [capability, temporal, pro]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# Animation API Contract

Capability flag for SDK/admin TimeSlider and playback integration.

| | |
| --- | --- |
| Capability key | `temporal.animation-api` |
| Category | Temporal |
| Edition | Pro |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/temporal.animation-api"` to the page that documents it.
