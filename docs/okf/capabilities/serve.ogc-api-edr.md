---
type: capability
title: "OGC API - EDR"
description: "Query environmental data resources through OGC API - Environmental Data Retrieval."
resource: "honua://capability/serve.ogc-api-edr"
tags: [capability, serve, community]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# OGC API - EDR

Query environmental data resources through OGC API - Environmental Data Retrieval.

| | |
| --- | --- |
| Capability key | `serve.ogc-api-edr` |
| Category | Serve |
| Edition | Community |
| Surface maturity | 6 preview |
| Registry entries | 6 |
| Proving tests | 31 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/serve.ogc-api-edr"` to the page that documents it.
