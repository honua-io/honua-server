---
type: capability
title: "OGC API Tiles"
description: "Serve vector and raster tiles through OGC API - Tiles."
resource: "honua://capability/serve.ogc-api-tiles"
tags: [capability, serve, community]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# OGC API Tiles

Serve vector and raster tiles through OGC API - Tiles.

| | |
| --- | --- |
| Capability key | `serve.ogc-api-tiles` |
| Category | Serve |
| Edition | Community |
| Surface maturity | 13 implemented |
| Registry entries | 13 |
| Proving tests | 67 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/serve.ogc-api-tiles"` to the page that documents it.
