---
type: capability
title: "OIDC Multi-Provider SSO"
description: "Configure multiple OIDC identity providers in one deployment."
resource: "honua://capability/identity.oidc-multi-provider"
tags: [capability, identity, enterprise]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# OIDC Multi-Provider SSO

Configure multiple OIDC identity providers in one deployment.

| | |
| --- | --- |
| Capability key | `identity.oidc-multi-provider` |
| Category | Identity |
| Edition | Enterprise |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/identity.oidc-multi-provider"` to the page that documents it.
