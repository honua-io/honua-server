---
type: capability
title: "OGC API Coverages"
description: "Serve coverage data through OGC API - Coverages."
resource: "honua://capability/serve.ogc-api-coverages"
tags: [capability, serve, community]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# OGC API Coverages

Serve coverage data through OGC API - Coverages.

| | |
| --- | --- |
| Capability key | `serve.ogc-api-coverages` |
| Category | Serve |
| Edition | Community |
| Surface maturity | 8 preview |
| Registry entries | 8 |
| Proving tests | 40 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/serve.ogc-api-coverages"` to the page that documents it.
