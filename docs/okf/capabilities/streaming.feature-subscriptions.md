---
type: capability
title: "Real-Time Feature Streams"
description: "Subscribe to WebSocket and SSE feature-change streams with filters and replay cursors."
resource: "honua://capability/streaming.feature-subscriptions"
tags: [capability, streaming, pro]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# Real-Time Feature Streams

Subscribe to WebSocket and SSE feature-change streams with filters and replay cursors.

| | |
| --- | --- |
| Capability key | `streaming.feature-subscriptions` |
| Category | Streaming |
| Edition | Pro |
| Surface maturity | 3 implemented, 2 preview |
| Registry entries | 5 |
| Proving tests | 112 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

## Documented in

- [React to feature changes](../../guides/edit/react-to-changes.md)
