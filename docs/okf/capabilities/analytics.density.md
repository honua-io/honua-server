---
type: capability
title: "Density Binning"
description: "Hex or square grid density (heatmap) binning over a filtered subset."
resource: "honua://capability/analytics.density"
tags: [capability, analytics, pro]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# Density Binning

Hex or square grid density (heatmap) binning over a filtered subset.

| | |
| --- | --- |
| Capability key | `analytics.density` |
| Category | Analytics |
| Edition | Pro |
| Surface maturity | 1 implemented |
| Registry entries | 1 |
| Proving tests | 10 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/analytics.density"` to the page that documents it.
