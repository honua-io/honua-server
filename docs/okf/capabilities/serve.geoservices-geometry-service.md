---
type: capability
title: "Geometry Service"
description: "Server-side geometry operations (project, buffer, simplify, union, and related utilities) through the Esri GeometryServer surface."
resource: "honua://capability/serve.geoservices-geometry-service"
tags: [capability, serve, community]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# Geometry Service

Server-side geometry operations (project, buffer, simplify, union, and related utilities) through the Esri GeometryServer surface.

| | |
| --- | --- |
| Capability key | `serve.geoservices-geometry-service` |
| Category | Serve |
| Edition | Community |
| Surface maturity | 48 implemented |
| Registry entries | 48 |
| Proving tests | 132 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/serve.geoservices-geometry-service"` to the page that documents it.
