---
type: capability
title: "OGC API Records"
description: "Search and retrieve catalog records through OGC API - Records."
resource: "honua://capability/serve.ogc-api-records"
tags: [capability, serve, community]
---
<!-- GENERATED FILE - DO NOT EDIT. Regenerate with scripts/ci/generate-capability-concepts.py -->

# OGC API Records

Search and retrieve catalog records through OGC API - Records.

| | |
| --- | --- |
| Capability key | `serve.ogc-api-records` |
| Category | Serve |
| Edition | Community |
| Surface maturity | 6 implemented |
| Registry entries | 6 |
| Proving tests | 41 |

The facts above come from `docs/gis/data/capability-keys.v1.json` and `capability-matrix.v1.json`, both generated from the server's own registry and test evidence. This page is a pure function of those two files — nothing in it depends on what the prose happens to say, so an unrelated documentation edit cannot stale it.

No page in this bundle declares this capability yet. Add `resource: "honua://capability/serve.ogc-api-records"` to the page that documents it.
