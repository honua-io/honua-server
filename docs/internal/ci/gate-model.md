...
4. **New checks default to nightly** unless explicitly justified as PR-blocking. Justification requires: deterministic behavior, sub-5-minute runtime, and author-actionable failure messages.
   ...
   **Named exception — `PR Gate` (#2865).** This rule governs *additional* checks joining the PR lane; `PR Gate` **is** the PR lane — the single required context on `trunk`. It is deterministic and author-actionable, but it does not meet the sub-5-minute bar: it compiles the whole solution warnings-as-errors, which dominates its runtime. That is accepted deliberately, because the only cheaper option is to gate nothing per-PR, and the only stricter option is the ~45-job matrix whose per-PR fan-out caused the 2026-06-18 runner-starvation spiral. Judge it on **runner count (one, service-free)**, not wall-clock. Any *further* PR-blocking check still owes the sub-5-minute justification above — and should first be considered as a step inside `.github/actions/lean-gate` rather than a new required context, since every extra context is another thing that can be required-but-never-reported (the #2865 failure mode).
