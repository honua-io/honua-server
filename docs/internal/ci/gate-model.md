...
| `review-gate.yml` | **Required admission context `Review Gate`.** Default-branch policy publishes exact-head Codex review evidence and, in enforce mode, releases one admitted PR Gate rerun. Review events arrive through the read-only `review-event-bridge.yml`; no PR-authored workflow has status or Actions write authority. | None — it must publish on every PR head |
