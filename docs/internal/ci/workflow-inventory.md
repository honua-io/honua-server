...
| `geoservices-import-fidelity-nightly.yml` | GeoServices Import Fidelity (External, On-Demand) | on-demand | `workflow_dispatch` | No | External parity vs live Esri services; run manually (upstream data drifts). Enforces both the correctness regression gate and the perf-parity latency gate (#1249) over the measured scorecard |
