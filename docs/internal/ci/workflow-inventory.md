...
| `parity-scorecard-governance.yml` | Parity Scorecard Governance | PR | `pull_request`, `workflow_dispatch` | Yes | Path-scoped to parity/baseline/perf-budget assets; smoke-tests the perf-parity gate (#1249) via pass/fail fixtures |
...
| `geoservices-parity-nightly.yml` | GeoServices Parity (External, On-Demand) | on-demand | `workflow_dispatch` | No | External parity vs live Esri services; run manually (upstream data drifts). Enforces both the correctness regression gate and the perf-parity latency gate (#1249) over the measured scorecard |
