...
|---|---|---|---|---|---|
| `pr-gate.yml` | PR Gate | PR | `pull_request` (base `trunk`), `workflow_dispatch` | Yes — required verification context | The cheap per-PR gate added by #2865, publishing `PR Gate` (never `CI Gate`). A single service-free ubuntu-latest runner performs the whole-solution build, format, Fast smoke, and architecture enforcement. In review-first enforce mode, attempt 1 stops before these expensive steps and the trusted reviewer releases attempt 2 exactly once. It is deliberately un-path-filtered. |
| `review-event-bridge.yml` | Review Event Bridge | PR | `pull_request_review`, `pull_request_review_comment` | No | GitHub runs these event workflows from the PR merge branch. This bridge is intentionally credential-free, performs no checkout, and exists only so its completion wakes trusted default-branch attestation. |
| `review-gate.yml` | Review Gate Attestation | PR | `pull_request_target`, `issue_comment`, PR Gate/bridge `workflow_run` | Yes — required admission context | Publishes `Review Gate` on the exact current head only when Codex has exact-head evidence and no unresolved Codex threads. It serializes every event by resolved PR number, runs only default-branch policy, and is the only authority allowed to release expensive verification. |
