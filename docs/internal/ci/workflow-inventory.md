...
| `stranded-merge-detector.yml` | Stranded Merge Detector | Maintenance | weekly `schedule`, `workflow_dispatch`, `workflow_call` | No | Read-only sweep for payload that never reached the default branch (#3248, #3316). Merged PRs whose merge commit is not an ancestor are adjudicated by **content** (blob equality, then presence of the PR's added lines) and split into `stranded` / `edits-missing` / `superseded` / `landed`; open PRs whose base has already landed or been deleted are reported as `needs-retarget` with the `gh pr edit` remedy. Files or updates a single tracking issue only on actionable findings. Reusable via `workflow_call` (`default-branch`, `limit`, `open-limit`, `tooling-ref`) and consumed by honua-sdk-js. |
