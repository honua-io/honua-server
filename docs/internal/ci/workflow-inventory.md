...
| `impact-routing-evidence-ledger.yml` | Impact Routing Evidence Ledger | Maintenance | daily schedule, `workflow_dispatch` | No | Read-only default-branch audit of stable-name PR Gate and native-image impact receipts. Counts only current trusted policy, distinct candidate heads; reconciles native decisions with successful exact-head Serving/GDAL image outcomes; and reports cohort readiness without changing routing, statuses, workflows, or merge state. |
