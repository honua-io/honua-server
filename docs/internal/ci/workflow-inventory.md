...
| Workflow file | Name | Triggers | Notes |
|---|---|---|---|
| `docs-quickstart-nightly.yml` | Docs Quickstart Validation | daily `schedule` (10:30 UTC), `workflow_dispatch` | Advisory execution of the primary documented Docker Compose quickstart. Uploads the bounded validation evidence for 14 days; `continue-on-error` keeps this wave-one signal observational while the workflow is hardened. |
