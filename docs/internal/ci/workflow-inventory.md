...
| `review-gate.yml` | Review Gate Attestation | PR | `pull_request_target`, `issue_comment`, trusted `repository_dispatch`, PR Gate/bridge `workflow_run` | Yes — required admission context | Publishes `Review Gate` on the exact current head only when Codex has exact-head evidence and no unresolved Codex threads. It serializes every event by resolved PR number, runs only default-branch policy, and is the only authority allowed to release expensive verification. Merge-train selection and pre-land independently re-attest source evidence. |
