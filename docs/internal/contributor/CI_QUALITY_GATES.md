...
Breaking changes require corresponding migration/deprecation documentation and the exact checked `OPENAPI_BREAKING_CHANGE_APPROVED` marker in the introducing PR. The marker keeps the workflow green but emits an Actions warning and a job-summary list of every suppressed finding. The repository-wide `OPENAPI_ALLOW_BREAKING_CHANGES` variable is a temporary pre-publication override governed by honua-server#3065 and the `honua-release#71` first-release gate; it must be `false` before publication and produces the same warnings while active.
