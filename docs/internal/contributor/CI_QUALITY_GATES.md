...
| Nightly slow tier | `nightly-slow-tier.yml` | `--filter "Tier=Slow&Category=Emulator"` across `Honua.Server.Tests`, `Honua.Db.Postgres.Tests`, `Honua.Core.Tests` — `[EmulatorTest]` only. Scale/Cloud/External slow subfamilies need dedicated workflows. |
| Nightly flake hunt | `flaky-detection.yml` | Re-runs a rotating window of shards (each under its own `test_timeout_minutes`, fresh DB per iteration) and reports outcomes that were inconsistent within one shard. Never fails on a candidate; fails when the evidence is incomplete. |
...
The shard map has a single source of truth: `.github/ci-shards.json` carries every shard's routing data (`paths`) and matrix-runtime metadata (`shard_name`, `filter`, `artifact_suffix`, `log_name`, `timeout_minutes`, `test_timeout_minutes`, `max_cpu_count`, upload flags). Adding or renaming a shard means editing one file. When a PR touches a source path under `unmapped_source_run_all_prefixes` (`src/Honua.Server/`, `src/Honua.Db.DuckDB/`, `tests/dotnet/Honua.Server.Tests/`) that no shard claims, the targeted-tests script emits `{"run_all": true, "reason": "unmapped_source_change"}` so new feature directories run on a full matrix until a follow-up PR adds explicit shard routing.
...
It also monitors shard headroom (#3054). Each run records `capacity_status` in the timing artifact and annotates the job: a passing shard that consumed at least `shard_budget_policy.warn_utilization` (80%) of its budget emits `::warning::HONUA_SHARD_LOW_HEADROOM`; a shard killed at the cap emits `::error::HONUA_SHARD_CAPACITY_EXHAUSTED` when it was still producing output, or `::error::HONUA_SHARD_HANG_SUSPECTED` when it had been silent for `HONUA_SERVER_TEST_STALL_SECONDS` (default 300). A shard that failed for any other reason is recorded as `not_assessed` and asserts nothing about headroom. Timeouts are recognised on both `timeout` exit codes — 124 when the test host dies from SIGTERM at the cap, and 137 when it ignores SIGTERM and has to be SIGKILLed after the `HONUA_SERVER_TEST_KILL_AFTER_SECONDS` grace; a 137 well inside the budget is instead reported as `HONUA_SHARD_KILLED` (typically an out-of-memory kill) so an external kill is never laundered into a capacity signal. The merge train reruns a suspected hang once, but capacity exhaustion gets its own terminal result code that skips both the rerun and per-PR attribution and escalates the whole batch, so a chronically full shard is fixed in `.github/ci-shards.json` instead of being retried and blamed on whichever PR happened to be in the batch. `scripts/ci/validate-ci-router.sh` additionally enforces that `timeout_minutes` clears `test_timeout_minutes` by `shard_budget_policy.min_job_overhead_minutes`, and `scripts/ci/audit-shard-headroom.py` re-bases budgets from collected timing artifacts. See [`docs/internal/ci/shard-timeout-budgets.md`](../ci/shard-timeout-budgets.md).
