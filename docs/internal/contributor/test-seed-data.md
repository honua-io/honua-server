...
| `tests/seed/client-compat-v1.sql` | Legacy versioned seed used by generic HTTP smoke, GDAL/PyQGIS, and Python STAC lanes; migration to the canonical fixture contract is tracked by #3393 | `client-compat-smoke-nightly.yml`, `client-interop-nightly.yml` (`gdal`, `pyqgis`), `docker/client-compat/seed/run.sh`, `tests/python/stac_client` |
