...
exact ruleset re-enable steps (a follow-up â€” **do NOT flip the ruleset in this
PR**), recommended queue sizing, and how the queue interacts with strict branch
protection and `pr-merge-train.yml`.

Companion docs:
- `docs/internal/ci/merge-queue.md` â€” the original CI-throughput operator runbook
  (shard routing, suite-speed, CI-minutes review). Its Â§1 "full lane on
  merge_group" design is **superseded** by this runbook's lean gate.
- `docs/internal/contributor/merge-coordination-runbook.md` â€” the webhook-driven
...
matrix** on the batched commit â€” because a `merge_group` event is a
...
cancelling the old** â€” zombie runs piling up â†’ spiral. Ruleset **17808547
"Merge queue (trunk)"** was set to **enforcement: disabled**.

## 2. The fix â€” a LEAN `merge_group` gate (this PR)
...
`merge_group` (see Â§5 for the exact graph). A batch of 5 therefore runs **one
runner job**, not ~225 â€” it finishes inside the timeout, so the front PR is
...
| **Merge queue** | `merge_group` | **`Merge Queue Gate` ONLY** â€” every other job cascade-skips | build + format + fast/arch unit smoke |

`CI Gate` is the **PR-time** required check. `Merge Queue Gate` is the
**queue-time** required check. They are distinct jobs by design.

## 3. Re-enabling ruleset 17808547 (FOLLOW-UP â€” after this lands on trunk)
...
1. **Make `Merge Queue Gate` the queue's required check.**
   - Branches â†’ ruleset **17808547 "Merge queue (trunk)"** â†’ **Merge queue**
     rule's **required status checks** â†’ set to **`Merge Queue Gate`**.
   ...
   - Keep `CI Gate` as the required check on the **branch-protection / PR**
     ruleset (the up-front gate before a PR can be queued) â€” unchanged.
2. **Sizing** (recommended):
   - `max_entries_to_build`: **2** (down from the prior 5). The lean gate is
     ...
     re-validate the queue. Raise to 3â€“5 once a week of clean runs proves the
     lean gate holds.
   ...
   - `check_response_timeout_minutes`: **15** (down from 60). The lean gate's
     ...
     are step caps but run sequentially â€” the job timeout is 30, so if you set
...
3. **Flip enforcement to `active`.** Set ruleset 17808547 enforcement from
   **disabled** â†’ **active** (Active). Optionally stage via **Evaluate** first
   to dry-run without enforcing.
4. **Verify live** (post-enable â€” the queue cannot be exercised until it's on
   the default branch):
   ...
   - Confirm a batch of â‰¥2 queued PRs produces **one** `merge-queue-gate` job
...
protection + `pr-merge-train.yml`, exactly as today. No code revert needed â€”
the lean gate simply stops being invoked when `merge_group` events stop firing.

## 4. Strict branch protection â€” keep it OFF
...
that the queue exists to eliminate â€” redundant and harmful. **Recommendation:
strict stays OFF.**
...
- **Job-graph trace (parsed `if:`/`needs:`):** on `merge_group`, the **only**
  job that runs is `Merge Queue Gate`. All 21 other jobs â€” including
  `aot-build`, `docker-build`, the `server-tests` shard matrix, the
  Postgres/browser/MCP/Python lanes, and `CI Gate` â€” cascade-skip (`changes`,
...
- **Concurrency:** `merge_group` keys on `github.ref`, which is the unique
  ...
  is not cancelled-by / cannot-cancel the prior batch â€” the lean gate makes the
  per-batch run cheap, so `cancel-in-progress: true` only collapses duplicate
  `checks_requested` events for the SAME batch ref.

**Live validation is post-merge + post-ruleset-enable** (Â§3 step 4), with the
one-toggle rollback in Â§3.
...
- **Exact prior ruleset field values** (`max_entries_to_build=5`,
  ...
  from `docs/internal/ci/merge-queue.md` Â§5, not re-read live from the GitHub
...
- The recommended `max_entries_to_build=2` and `check_response_timeout=15` are
  conservative re-entry values, not measured â€” tune up after a week of clean
