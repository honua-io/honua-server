...
- **Ai reaches the Geocoding and Routing satellites (#1597).** The MCP
  ...
  as the Protocols -> Routing edge the GeoServices NAServer adapter uses.
  Neither satellite references Ai, so both edges are acyclic. (`Honua.Routing`
  predates a dedicated column in the table above; the enforced source of truth
  is the tuple matrix in `ModuleDependencyPolicyTests`.)
