...
Evidence bundle used for this refresh:
...
artifact nevertheless reported `allPassed=true` with 1138 passed tests, 0
failed, 0 skipped, and 0 CantTell across 14 suites. Treat that artifact as the
conformance result, not the failed workflow attempt as run-level proof.
