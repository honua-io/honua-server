...
completed on 2026-08-31 against
`trunk@a5fb7e4511fb08dcd17e6c2cf2bf9d52d1c3cef8`. It reports 166/167: Honua
rejects multi-layer transactions when `rollbackOnFailure=true` because
cross-layer atomicity is not yet guaranteed. The remaining suites use the
aggregate evidence bundle from
[`CITE Evidence Report` run 33205558805](https://github.com/honua-io/honua-server/actions/runs/33205558805)
completed on 2026-08-28T20:25:07Z against trunk commit
`f5ac595ee5e3d4bce3df3c726ca1127ca4e2da0f`. The workflow run concluded with
failure because its final status-freshness check detected the then-stale
checked-in snapshot. Its successfully generated `cite-conformance-evidence-13`
artifact reported `allPassed=true` with 1138 passed tests, 0 failed, 0 skipped,
and 0 CantTell across 14 suites before the later WFS regression. Treat the newer
per-suite WFS run as authoritative where the receipts differ.
...
all zero. A partial result remains published as partial, with its failing
requirement and receipt, and must not be described as fully compliant.
...
The explicit WFS 2.0 transactional evidence leg exercises the Transaction and
LockFeature conformance classes independently from the `basic` leg.

WMS 1.1.1 and WMS 1.3 `default` evidence run their official ETS profiles
against the same seeded map service. The WMS 1.1.1 leg also verifies legacy
version negotiation, request aliases, axis order, exception formats, and GML
...
HONUA_CITE_WFS20_RESULTS_DIR=cite-wfs20-transactional-results \
  scripts/conformance/cite/run-cite-wfs20-tests.sh --profile transactional --verbose
scripts/conformance/cite/run-cite-wcs20-tests.sh --profile core --verbose
scripts/conformance/cite/run-cite-wms11-tests.sh --profile default --verbose
...
summary. The authoritative aggregate run above passed 21/21 selected Basic plus
Asynchronous assertions and 22/22 raw assertions when the unselected Synchronous
class is included. WPS is included by `build-cite-evidence.sh` and the
weekly/manual aggregate workflow.
