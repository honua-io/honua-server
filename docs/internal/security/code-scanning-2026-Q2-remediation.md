...
| `src/Honua.Db.DuckDB/Features/FeatureStore/Services/DuckDBFeatureDataAccess.cs:448` | `cs/sql-injection` | dismiss | `ParameterizedQuery.Sql` is built from a fixed template plus positional `?` placeholders; user values bind as parameters in the loop below. In-source `// codeql[cs/sql-injection]` annotation present. |
