...
- Not yet complete: demo-grade 3D Tiles generation, point-cloud/reality-capture ingest (spike recommends pre-tiled 3D Tiles registered through the existing scene dataset registry as the first slice, with bounded follow-ups for COPC streaming and CPU/GPU PDAL conversion; see [`gis/point-cloud-reality-capture-ingest.md`](../spikes/point-cloud-reality-capture-ingest.md)), native USD geometry conversion/Omniverse/Unreal integration beyond the Pro preview USDA stage manifest, and a unified construction world-model fixture remain backlog work.
...
- **GeoETL** (epic `#361`): scheduled, repeatable, multi-source spatial extract-transform-load pipelines as a Pro/Enterprise capability layered on the durable job substrate. See [ADR-0038](../contributor/adr/0038-geoetl-pipeline-architecture-and-runtime-boundary.md) for the runtime boundary, worker profile, and binding architectural decisions. Community continues to ship one-shot file import unchanged.
