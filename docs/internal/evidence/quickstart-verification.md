# Quickstart clean-room verification (internal)

The customer quickstart used to be this: a single PowerShell program that set
NTFS ACLs on the install directory, generated secrets through
`System.Security.Cryptography.RandomNumberGenerator`, polled readiness in an
embedded Python heredoc, asserted that anonymous admin access is denied, and
compared every published ordinate to 1e-9. All of that is real and worth
running - as a release rehearsal, which is what it was written as. It is not
what somebody trying the product for the first time should meet, and it shut
out every reader not on Windows.

Preserved verbatim below. `docs/get-started/quickstart.md` is now a
shell-neutral Docker Compose journey.
...
For a Linux shell, use the equivalent [Linux package quickstart](../../get-started/linux-packages.md).
