...
- The hosted serving path already accepts the canonical 3D Tiles MIME types
  ...
  [Hosted 3D Tiles Scenes](../../guides/publish/publish-3d-scenes.md#mime-types)). `.pnts` covers the
...
| **Orthoimagery (GeoTIFF / COG)** | Raster | Use the existing [raster pipeline](../../guides/publish/publish-rasters.md), not this spike | COG-direct serve or registered raster | RAPIDS / cuSpatial post-process | Out of scope here; orthoimagery flows through the COG path, not 3D scenes. |
...
| Domain placement | Entities live in `Honua.Core/Features/Scene/Domain/` (or a sibling `Capture/Domain/`); persistence and repository implementations stay in `Honua.Db.Postgres` as `internal sealed`. `Honua.Core` must not reference `Honua.Server` or provider projects. |
