...
- `layers` (preferred) or `layerIds` (legacy alias): comma-separated layer ids. The list must contain at least one parseable id — inputs that resolve to zero ids (e.g. `?layers=,`) are rejected with `400 Invalid layer filter: layers/layerIds must specify at least one layer ID.` so a malformed filter cannot bypass the unfiltered admin/auth gate by faking a "filtered" subscription. Each layer is access-checked against both its layer policy and (when no `serviceId` is supplied) its primary service policy, matching the OGC API Collections enforcement pattern.
...
Cross-node recovery (`CrossNodeSyncInterval`) covers all WebSocket subscriptions, not only the session-managed default. Each interval the writer polls the durable store for events past each non-paused subscription's per-subscription poll watermark and re-runs the per-(event, subscription) atomic dedup claim. The watermark is seeded after the subscribe-time replay (or, for no-cursor subscribes, from the store cursor captured before the subscription is added) and is then advanced after every successful live send to that subscription, so the next poll only sweeps events strictly past what has already been delivered. The default subscription's cursor fence (the writer's local `replayCursor`) follows the same advance-on-live-send pattern as the SSE drain. This is the recovery path for events that landed in the durable store on another node but were never delivered through the local broadcast / Redis pub/sub fan-out.

The poll runs on an absolute-deadline schedule rather than a per-iteration delay: the writer tracks `nextPollAt = lastPollAt + CrossNodeSyncInterval` and uses the remaining time as the WhenAny delay. After draining a burst of channel traffic the writer also runs the poll inline if the deadline has elapsed. Without these guarantees, a session whose channel is continuously hot would always lose the WhenAny race to the read branch and the durable-store sweep would never fire.
