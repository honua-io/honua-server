...
Use an `ops:read` credential for reads. Proposal and approval calls require the
corresponding admin/operator permissions, and the proposer cannot approve their
own proposal. The authorized
[API explorer](../../reference/openapi-and-explorer.md) exposes every REST
operation named below from the generated contract.

## 1. Observe

In the API explorer, run `GET /api/v1/operate/status` and
...
Run `GET /api/v1/admin/observability/findings` in the API explorer.
...
For a finding that contains a recommended action, run
`POST /api/v1/admin/observability/findings/{findingId}/propose` in the API
explorer.
...
`GET /api/v1/admin/proposals/{id}` and approve in Console's inbox or through the
API explorer's `POST /api/v1/admin/proposals/{id}/approve` operation.

## 4. Verify the result

Run `GET /api/v1/admin/deploy/operations/{operationId}` and
`GET /api/v1/admin/observability/ops-health/history` in the API explorer.
