...
| `503` on OGC Processes or GPServer job routes | Redis not configured | Enable Redis — see [Infrastructure](kubernetes.md) |
...
| Symptom | Likely cause | Fix |
|---------|-------------|-----|
| `503` on cancel for Orchestration operations | Redis not configured; orchestration engine not registered | Enable Redis — see [Infrastructure](kubernetes.md) |
...
For production incidents, see the [Runbooks](../../archive/role-indexes/operator-runbooks-README.md).
