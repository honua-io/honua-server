...
> [Deployment Scenarios](deployment-scenarios.md#apiworker-host-separation).
> See [ADR-0031](../../internal/contributor/adr/0031-durable-job-orchestration-substrate.md).
...
[Console Job Observability](../../internal/admin-api/console-job-observability.md) for the
...
- `GET /api/v1/admin/jobs` — cursor-paginated durable job history and filters
- `GET /api/v1/admin/jobs/{jobId}` — durable job detail
...
The mapping between admin operation tracking and GPServer responses is:

| Admin operation state | GPServer `jobStatus` | Endpoint |
|-----------------------|----------------------|----------|
| Queued | `esriJobSubmitted` | `submitJob` returns 200 |
