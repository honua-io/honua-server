...
The self-hosted rolling backend (`honua-yarp-rolling`) stages the candidate as a standby replica that serves no traffic until the proxy swaps to it, so its exposure starts at the cutover. Before the cutover, only the checks that need no candidate traffic run: the backend's standby health gate, plus `telemetry.healthz.url` and the golden query when you set them (point them at the standby replica). Those checks are bounded by `telemetry.exposure_deadline_seconds`; a standby that never passes them is rolled back without being activated. Error-rate, latency, and sample-floor metrics are evaluated from the cutover, inside the post-activation observation window below.
