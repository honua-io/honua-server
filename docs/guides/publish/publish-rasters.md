...
samples and `format=jpg` for JPEG tiles. Register against a layer without imported
...
- **Floating-point or JPEG source rejected** — TIFF floating-point predictor 3 is unsupported; prepare floating-point sources without that predictor (predictor 1). Complex or undefined TIFF SampleFormat values and malformed JPEGTables are rejected during metadata extraction; JPEG tiles other than 8-bit chunky grayscale, RGB or YCbCr are not served. Use imported rasters for these source layouts.
