...
| `sdk-server-compatibility.yml` | SDK Server Compatibility | nightly | `schedule`, `workflow_dispatch` | No | Weekly live compatibility smoke against checked-out `honua-sdk-js`, `honua-sdk-python`, and `honua-sdk-dotnet`; validates admin compatibility metadata plus seeded FeatureServer and OGC API Features surfaces through SDK clients |
