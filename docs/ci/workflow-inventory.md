...
| `nightly-slow-tier.yml` | Nightly Slow Tier | nightly | `schedule`, `workflow_dispatch` | No | Daily 4:00am UTC; runs `--filter "Tier=Slow"` across `Honua.Server.Tests`, `Honua.Postgres.Tests`, and `Honua.Core.Tests` (covers `[ScaleTest]`, `[ExternalServiceTest]`, `[EmulatorTest]`, `[CloudTest]`); asserts presence of `HONUA_TEST_S3_*`, `HONUA_TEST_AZURE_BLOB_*`, and `HONUA_TEST_DB_URL` before dispatch so missing env vars fail loudly. ADR-0035 |
