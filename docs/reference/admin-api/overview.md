...
- [Control Plane Versioning Policy](../versioning-and-support.md)
...
For Esri File Geodatabases, use a `.gdb.zip` archive that contains the `.gdb` directory and preserves the directory structure inside the archive. See [FileGDB Import Workflow](../../guides/publish/filegdb-import-workflow.md).
...
[ArcGIS Inventory Discovery](../../guides/migrate/arcgis-inventory-discovery.md), including the
...
Compatibility assessments may include a stable, machine-readable `code` (for example `COMPATIBLE`, `MANUAL_REVIEW`, `ARCGIS_UNSUPPORTED_RENDERER`, `ARCGIS_TOKEN_REQUIRED`) alongside `level`, `reason`, `warnings`, and `manualSteps` when the scanner can assign one deterministically. The ArcGIS GeoServices code namespace and remediation table are documented in [ArcGIS Inventory Discovery — Compatibility Codes](../../guides/migrate/arcgis-inventory-discovery.md#compatibility-codes); `code` is omitted for aggregate assessments where no single code applies and for source-specific assessments that do not yet define a stable code.
...
The public `GET /api/styles/{layerId}.json` endpoint accepts an optional `?theme=default|dark|colorblind-safe|print` query parameter for deterministic theme transforms. The output cache varies by `theme`; the admin update endpoint invalidates every variant on each revision. See [Style Engine: Cross-Protocol Consumption](../../guides/style/style-maps.md) for the full contract and how MVT, MapServer, and WMS consume the canonical document.
...
| `/api/v1/admin/metadata/layers/{layerId}/style/import-sld` | POST | Convert an SLD/SE 1.0 or 1.1 XML document to MapLibre style JSON and store it (admin only, Community edition; 1 MiB body cap). See [SLD Migration Reference](../../guides/style/import-sld-styles.md). |
...
[Metadata Prevalidation Admin API](../../internal/admin-api/metadata-prevalidation.md) for
the full contract.
...
documented in [Console Job Observability](../../internal/admin-api/console-job-observability.md).
See [Operations - Job Orchestration](../../guides/deploy/operations.md#job-orchestration) for
...
[Operations — Workflow Orchestration](../../guides/deploy/operations.md#workflow-orchestration)
...
[Upgrade and Rollback Runbook](../../guides/deploy/upgrade-and-rollback.md) for per-backend
...
[Progressive Multi-Step Canary Ramp](../../guides/deploy/upgrade-and-rollback.md#progressive-multi-step-canary-ramp)
...
[ADR-0033](../../internal/contributor/adr/0033-unified-license-format.md) and the companion
[unified license and entitlement architecture](../../internal/contributor/architecture/unified-license-and-entitlement.md).
Operational procedures live in the licensing runbooks:
[License Migration](../../concepts/license-migration.md),
[License Key Rotation](license-key-rotation.md), and
...
[Tile Operations Runbook](../../guides/publish/publish-tiles.md) and the
[PMTiles Publishing](../../guides/publish/pmtiles-publishing.md) guide for request/response details
...
`honua://jobs/{jobId}/report` (see [MCP Server](../../guides/connect/ai-agents-mcp.md)).
...
[Analysis Content](../../internal/admin-api/analysis-content.md).
...
- [Operator Guide](../../archive/role-indexes/operator-README.md)
...
- [Security](../../guides/secure/authentication.md)
- [Control Plane Versioning Policy](../versioning-and-support.md)
- [Control Plane Migration Guide](../control-plane-migration-guide.md)
- [Upgrade and Rollback Runbook](../../guides/deploy/upgrade-and-rollback.md) — deploy backend configuration for AWS Lambda, Azure Functions, Azure Container Apps, AWS ECS + ALB canary, and Kubernetes
