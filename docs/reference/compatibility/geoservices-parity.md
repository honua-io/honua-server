...
| `GET /services?wsdl` | Implemented | Public WSDL for the root-only ArcGIS service catalog. It publishes document/literal SOAP 1.1 (`text/xml`) and SOAP 1.2 (`application/soap+xml`) bindings at the same externally resolved `/services` address. A GET without the `wsdl` query flag returns 404. |
...
- **Not to be confused with** the import-fidelity scorecard workflows/tests (`import-fidelity-scorecard-governance.yml`, `geoservices-import-fidelity-external.yml`, `GeoservicesImportFidelityIntegrationTests`, `import-fidelity-scorecard-baseline.json`), which measure *data-import fidelity* across ten dataset cases and never read this matrix. They are a different question that once shared a confusingly similar name; that side was renamed off `parity` by [#2861](https://github.com/honua-io/honua-server/issues/2861).
