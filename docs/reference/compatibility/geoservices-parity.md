...
| [MapServer](#mapserver--wms--wmts) | Partial | Export, identify, find, legend/queryLegends, query, tiles with cloud-storage cache, storage-backed exportTiles, generateKml, WMS 1.3/1.1.1, WMTS 1.0 | Full Esri tile-package/job export semantics, dynamicLayer child resources/workspace dynamic layers, several child resources; WMTS is WebMercatorQuad-only |
...
| generateRenderer (service-level), queryAnalytic, dynamicLayer child resource, image/KML-image/job child resources, `exts/*` | Not implemented | |
