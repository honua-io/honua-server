...
- **Not to be confused with** the `parity-scorecard-*` workflows/tests (`parity-scorecard-governance.yml`, `geoservices-parity-nightly.yml`, `GeoservicesParityIntegrationTests`, `parity-scorecard-baseline.json`), which measure *data-import fidelity* across ten dataset cases and never read this matrix. They are a different question with a confusingly similar name; renaming that side is tracked by [#2861](https://github.com/honua-io/honua-server/issues/2861).
