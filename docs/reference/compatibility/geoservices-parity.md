...
| WMTS | Partial | GetCapabilities/GetTile (KVP + RESTful) at `.../MapServer/WMTS` and `/ogc/services/{serviceId}/wmts` for both the WebMercatorQuad and WorldCRS84Quad (CRS84/EPSG:4326) gridsets; GetFeatureInfo is WebMercatorQuad-only. WMTS 1.0 is CITE-certified (60/60). |
...
| tile, computeHistograms, getSamples | Implemented | PNG/JPEG/TIFF tiles, zoom 0–28, multi-raster mosaic. Dynamically generated tiles auto-apply a MinMax display stretch when the source raster is not 8-bit (elevation/analytic/float), so analytic layers are viewable; at low zoom levels the tile read path prefers a persisted reduced-resolution overview pyramid (built at import in EPSG:3857; level chosen by the shared overview-resolution scorer, reused from the COG read path) and otherwise coarsens the source on-the-fly toward the tile's ground resolution before resampling (a no-op at native/finer zoom; can only downsample, never upsample); pre-rendered cached tiles are served as stored. Tile GETs use configured cloud file storage (`local`, S3, Azure Blob) as a deterministic loose-object read-through/write-through cache; getSamples caps at 1000 samples. getSamples reads from the registered raster catalog (PostGIS); per-slice pixel subsetting of registered multidimensional cubes (Zarr/HDF5/NetCDF) by `multidimensionalDefinition` (time/StdZ coordinate) is deferred — those coverages are metadata-only today, so a dimension-value sample selector would have nothing to read. |
...
sort order), `esriMosaicNorthwest` (the upper-left-most raster wins),
...
and non-date `esriMosaicByAttribute` return 501 when more than one raster is
selected. Temporal `time` filters use newest-batch semantics (see
