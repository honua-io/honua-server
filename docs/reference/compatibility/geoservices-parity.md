...
| [NAServer](#naserver) | Partial | Route solve and service-area solve over the shared routing provider, incl. point/line/polygon barriers and validated `travelMode`; minimal ClosestFacility probe envelope for mobile clients | POST-only; ClosestFacility is a deterministic stub; no OD cost matrix or location-allocation; only the driving travel mode is genuinely routable (single stored cost weight) |
| Portal Sharing | Partial | `generateToken` opaque tokens consumable on `/rest/services/*`; OAuth2 **named-user** flow at `/sharing/rest/oauth2/{authorize,callback,token}` — `authorization_code` + PKCE and rotating `refresh_token` (OIDC-delegated) — plus the opt-in `client_credentials` (service-to-service) grant | item/group/community sharing surface — see [authentication](../../guides/secure/authentication.md) |
...
| queryContingentValues | Implemented | Serves per-layer contingent value definitions (cross-field value-combination constraints) from the Metadata v2 graph (`contingentValueGroups` on each resource); layers with none, and services with none, return an empty `contingentValuesDefinitions` collection. Authored through the layer metadata graph; automated import of contingent values from Esri sources is not yet wired (deferred). |
...
| `returnZ`, `returnM` (3D/measured queries) | Implemented (Postgres) | `returnZ`/`returnM` carry the higher ordinates through the storage read: the Postgres geometry read switches to extended WKB (EWKB) when either flag is set, so stored Z/M survive to the output and are emitted on the GeoServices JSON/PBF geometry; without the flag the read stays 2D OGC WKB (byte-identical to prior behavior). Layers advertise `hasZ`/`hasM` from authored metadata. GeoJSON output still rejects `returnM=true` (RFC 7946 has no M); `parquet`/`arrow` strip M. |
| `returnTrueCurves` | Partial | Accepted and ignored for client compatibility; Honua still advertises `supportsTrueCurve=false` and emits linearized geometries. True curve (circular-arc/Bézier) parsing, storage, and re-emission are deferred (`Refs #1877` Part B) — NTS/WKB cannot represent true curves without a sidecar curve-storage design. |
...
| tile, computeHistograms, getSamples | Implemented | PNG/JPEG/TIFF tiles, zoom 0–28, multi-raster mosaic. Dynamically generated tiles auto-apply a MinMax display stretch when the source raster is not 8-bit (elevation/analytic/float), so analytic layers are viewable; at low zoom levels the source raster is coarsened on-the-fly toward the tile's ground resolution before resampling (a no-op at native/finer zoom; can only downsample, never upsample; persisted COG-style pyramids deferred); pre-rendered cached tiles are served as stored. Tile GETs use configured cloud file storage (`local`, S3, Azure Blob) as a deterministic loose-object read-through/write-through cache; getSamples caps at 1000 samples. getSamples reads from the registered raster catalog (PostGIS); the `multidimensionalDefinition` parameter (per-slice pixel subsetting of a registered multidimensional cube by a time/StdZ coordinate) is now parsed and validated (#1869) — a malformed definition returns 400 — but per-slice pixel resolution remains deferred: an ImageServer layer exposes multidimensional descriptor metadata only and has no readable per-slice backing store wired into the getSamples path, so any supplied `multidimensionalDefinition` returns a clean 501 rather than silently sampling the collapsed raster. A genuine implementation depends on a layer→Zarr coverage mapping and a non-time (StdZ) coordinate-value→index resolver (only the time axis is resolvable today). |
...
unsupported), `esriMosaicNorthwest` (the upper-left-most raster wins), and
...
`ST_Union`). `esriMosaicSeamline`, `esriMosaicNadir`, and `esriMosaicByAttribute` over
a non-allowlisted (e.g. sensor/orientation) field return 501 when more than one raster
is selected — `esriMosaicNadir` in particular stays deferred until sensor/orientation
metadata is modeled. Temporal `time` filters use newest-batch semantics (see
...
| Operation(s) | Status | Notes |
| --- | --- | --- |
| Route/solve | Implemented | Parses `stops`, `inSR`/`outSR`, `returnRoutes`, `returnDirections`, `barriers`/`polylineBarriers`/`polygonBarriers`, and `travelMode`; delegates to the configured routing provider and returns Esri route/directions feature sets. |
...
| Service metadata, OD cost matrix, location-allocation, network-dataset editing | Not implemented | Route and service-area solves (with barriers and validated travel mode) are the routing scope. Unsupported provider capabilities return GeoServices 400 error envelopes rather than fabricated solves. |
