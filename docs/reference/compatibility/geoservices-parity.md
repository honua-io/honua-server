...
| WMTS | Partial | OGC WMTS 1.0 GetCapabilities/GetTile (KVP + RESTful) at `.../ImageServer/WMTS` for numeric and service-name ImageServer routes. Uses the ImageServer tile/cloud-cache pipeline, advertises `WebMercatorQuad`, and serves `image/png` and `image/jpeg` tiles. GetFeatureInfo, non-WebMercator matrix sets, dimensions, and other tile formats (TIFF) are deferred. |
