...
| computeClass (Honua extension) | Implemented (validation only) | Validates `renderingRule` raster-function chains (`Identity`, `Stretch`, `Colormap`, `Clip`, `ExtractBand`, depth ≤ 8) and returns planned execution metadata. `exportImage` and `identify` execute the `Stretch`, `Colormap` (explicit stops, named `ColorrampName`, or an inline algorithmic/multipart `Colorramp` object), `Clip` (including `ClippingType=1` keep-outside), and `ExtractBand` portions of a chain. |
