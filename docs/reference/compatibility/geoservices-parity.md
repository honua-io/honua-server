...
| [ImageServer](#imageserver) | Partial | Service metadata, exportImage, identify, tile, catalog query, statistics/histograms, getSamples, queryBoundary, computePixelLocation, project, storage-backed exportTiles, dynamic computeCacheInfo, legend, WCS 2.0.1 KVP | Catalog mutation, full Esri tile-package/job export semantics, measure, generated ImageServer tile caches, renderingRule execution, AOI clipping for stats, WMTS |
...
| addRasters, deleteRasters, updateRaster, uploads, downloadRasters, find, measure, calculateVolume, computeMultidimensionalInfo, computeTiePoints, validate | Not implemented | Raster ingestion happens through the Honua admin API (`/api/v1/admin/import/raster`, `/api/v1/admin/cloud-rasters`) instead. |
