...
Which operations exist is derived from the server's own endpoint registry, so nothing is
listed here that Honua does not serve and nothing served is omitted. How completely each
operation is implemented is a human judgement, recorded alongside the machine-readable
matrix and checked against the registry on every build.
...
| GeocodeServer | Partial | Service metadata, findAddressCandidates, reverseGeocode (incl. provider-dependent `distance`/`featureTypes`), suggest, geocodeAddresses, `outFields` projection, `outSR` reprojection, `magicKey` suggest→candidate round-trip (self-issued signed token, all providers), `category` filtering (all providers, on provider-supplied address type) | `forStorage`/`matchOutOfRange` (re-deferred; no backing provider models them) |
| GPServer | Partial | PrintingTools; generic adapter over 98 seeded processes with catalog-backed task metadata, 37 additive Esri-conventional name aliases, async submitJob, synchronous `execute` for the deterministic single-geometry `geometry.*`/`conversion.geometry-format` family (inline over the canonical job runtime), job status/cancel, and durable named-result retrieval | Aliases retain canonical Honua parameters rather than adapting the same-named Esri tool's wire contract; heavyweight/layer-scoped tasks stay async-only (their `execute` returns a 400 pointing at submitJob); both routes honor `env:outSR`, `env:workspace`, and `env:overwriteOutput`, while `env:processSR` is accepted/preserved but not yet applied and other `env:*` controls return 400 — see [run geoprocessing](../../guides/query-analyze/run-geoprocessing.md) |
| [NAServer](#naserver) | Partial | GET/POST route and POST-only service-area, closest-facility, OD-cost-matrix (cost-only), and location-allocation (minimize-impedance / maximize-coverage) solves over the shared routing provider, incl. point/line/polygon barriers and dataset-backed `travelMode`; addressable network-dataset registry with admin editing of the dataset mapping/metadata | OD-matrix geometry output, additional location-allocation problem types, and edge/vertex/turn-restriction feature editing with topology rebuild are deferred; the built-in topology remains driving-only unless operators map additional verified cost columns |
...
| [Portal Sharing](#portal-sharing) | Partial | `generateToken` opaque tokens consumable on `/rest/services/*`; `/sharing/rest/info` auth discovery; OAuth2 **named-user** flow at `/sharing/rest/oauth2/{authorize,callback,token}` — `authorization_code` + PKCE and rotating `refresh_token` (OIDC-delegated) — the opt-in `client_credentials` grant with optional IdP/OIDC federation, optional JWT access tokens + RFC 7662 `oauth2/introspect` (#1890, opt-in), RFC 7009 `oauth2/revoke`, and read/search/community projections over the Metadata v2 graph | Portal/community `self` and item documents are partly hardcoded (portal id/name, `role`, item `owner`); `search` honours a small `q` grammar and ignores `bbox`/`filter`/`categories`; the group + item-sharing overlay is **in-memory, single-node, and never read back** — sharing an item or joining a group changes nothing any client can observe; `/content/items/{id}/data` returns the item document, not the data payload — see [authentication](../../guides/secure/authentication.md) |
...
These server integration checks do not establish a native desktop certification pass.
...
| queryRelatedRecords, queryAttachments | Implemented | Full filter facets (`attachmentTypes`, `keywords`, `size`, `definitionExpression`); `globalIds` rejected with 400 (integer object IDs only). Attachment parent visibility resolves through the publication's storage binding, including queries without `definitionExpression`. Per-feature attachment reads and mutations use the same bound reader for parent existence and owner-policy checks; stale rows in the default snapshot cannot authorize access to a deleted bound parent. Canonical composite relationships retain `composite: true` in layer metadata and require reciprocal declarations. They are read-only: shared resource/write authorization denies mutations even with write grants, and graph validation rejects enabling edits. Automated composite migration and editable ownership/cascade semantics remain unsupported. |
...
| `returnExceededLimitFeatures` | Accepted (no-op) | Accept-and-ignore for interop: the ArcGIS Maps SDK for .NET always sends it. Honua already returns the truncated page plus `exceededTransferLimit=true`, so both the default and an explicit `false` return the same page and flag. |
...
| `returnTrueCurves` | Partial | **Input** is densified: true-curve geometry supplied as `curvePaths`/`curveRings` on `applyEdits` or a query spatial filter is densified to linear vertices before storage — circular-arc (`{"c":…}`), center-form circular/elliptic-arc (`{"a":…}`), and cubic-Bézier (`{"b":…}`) segments are interpolated with a bounded vertex count; generated vertices preserve and interpolate Z/M ordinates. **Output** stays linear: NTS/WKB cannot represent a true curve, so densified geometry is what is stored and queried back — there is no `curve` re-emission from stored linear geometry. The parse↔serialize round-trip is proven at the converter level (`CurveGeometryConverter.Parse`/`Serialize`). On query, `returnTrueCurves=true` is accepted but returns the densified linear geometry with correct `hasZ`/`hasM`; Honua still advertises `supportsTrueCurve=false`. Lossless curve storage/re-emission remains deferred (requires a sidecar curve-storage design). |
...
failure without creating duplicate features. When the header is present, the first
...
`synchronizeReplica` uploads (`syncDirection=upload` or `bidirectional`) are at-most-once too. Field clients on flaky links re-send an upload whose response they never received, and
...
- An upload that fails after committing some rows is recorded too. This happens only in the opt-in
  ...
  an upload that omits it rolls back each failing layer's whole batch. Its retry returns the
...
| image/KML-image child resource (`/ImageServer/kml/image.kmz`) | Implemented | Service-level KML2/KMZ resource for the ImageServer extent. The payload is an OGC KMZ (doc.kml) with one GroundOverlay and an icon href back to the service-level `exportImage` route. |
| raster item metadata XML (`{rasterId}/info/metadata`) | Not implemented | Source-native metadata and support files require an explicit canonical storage contract; Honua does not synthesize or leak provider files when that source data is unavailable. |
...
`esriMosaicNadir` (the raster acquired closest to straight-down wins,
...
| Operation(s) | Status | Notes |
| --- | --- | --- |
| Root `GeometryServer` metadata | Implemented | Descriptor with `serviceDescription`, `maxBufferCount`, and `maxSimplifyCount`; it intentionally omits `currentVersion`. |
...
`Route/solve` accepts GET query parameters or POST form parameters; `ServiceArea`, `ClosestFacility`, `ODCostMatrix`, and
...
exactly as Esri clients send them. Only the inputs the solve operation consumes are read, and `latestWkid`
...
| `resources/sprites/{spriteResource}` | Stub | Routes exist and return spec-shaped assets with no content: `sprite.json`/`sprite@2x.json` return `{}` and `sprite.png`/`sprite@2x.png` a 1x1 transparent PNG (`@2x` returns the identical 1x asset). No sprite store backs them. |
...
| Operation(s) | Status | Notes |
| --- | --- | --- |
| `GET /rest/services` | Implemented | Graph-backed service directory, RBAC-filtered per principal, listing every supported Esri service type a publication exposes. When every publication is denied, the GeoServices JSON envelope carries ArcGIS error code `499` (anonymous) or `403` (authenticated) and no service entries. Does not emit `folders`. |
| `GET /services`, `GET /services?wsdl` | Implemented | Public WSDL for the root-only ArcGIS service catalog. It publishes document/literal SOAP 1.1 (`text/xml`) and SOAP 1.2 (`application/soap+xml`) bindings at the same externally resolved `/services` address. The bare site-root form returns the same document as the `wsdl` query flag, so ArcGIS Pro's site-root server connection resolves. |
| `POST /services` | Implemented | Supports `GetServiceDescriptions`, `GetServiceDescriptionsEx`, `GetFolders`, `GetMessageVersion`, `GetMessageFormats`, `GetTokenServiceURL`, and `RequiresTokens` for SOAP 1.1 and 1.2. Discovery uses the exact REST directory projection, including routability, RBAC, licensing, ImageServer availability, and public-origin resolution. `FeatureServer`, `MapServer`, `ImageServer`, `GPServer`, `VectorTileServer`, and Enterprise `SceneServer` are eligible; other types are omitted. Every record carries `RestUrl`; ImageServer retains its implemented SOAP `Url`, while REST-only types use the REST handoff for both. Denied-only discovery returns a SOAP fault with HTTP 401 (anonymous) or 403 (authenticated); malformed/unsupported requests return a version-matched sender/client fault. Named folders return an empty catalog because Honua exposes only the root folder. |
...
Only `/rest/info` advertises the compatibility value `currentVersion: 10.8`.
...
| `oauth2/token` | Implemented | `authorization_code` and `refresh_token` grants with rotating refresh tokens; Basic-auth client credentials accepted. POST only. The `client_credentials` grant is opt-in (default off, returns `unsupported_grant_type`), as is its IdP/OIDC federation. |
| `oauth2/introspect` | Implemented (off by default) | RFC 7662 over opaque, JWT, and refresh tokens, admin-authorized; 404 when disabled. Emits `active`, `sub`, `username`, `scope`, `token_type`, `exp`; `client_id`, `iat`, `nbf`, `aud`, `iss`, `jti` are not emitted. |
...
- [Run geoprocessing](../../guides/query-analyze/run-geoprocessing.md) and [authentication](../../guides/secure/authentication.md) — drill-downs for the services not detailed on this page.
