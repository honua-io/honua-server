...
| [MapServer](#mapserver--wms--wmts) | Partial | Export, identify, find, legend/queryLegends, query, tiles with cloud-storage cache, storage-backed exportTiles, generateKml, WMS 1.3/1.1.1, WMTS 1.0 | Full Esri tile-package/job export semantics, dynamic layers, several child resources; WMTS is WebMercatorQuad-only |
| [ImageServer](#imageserver) | Partial | Service metadata, exportImage, identify, tile with cloud-storage cache, catalog query, find, measure (Basic), statistics/histograms, getSamples, queryBoundary, computePixelLocation, project, storage-backed exportTiles, dynamic computeCacheInfo, legend, WCS 2.0.1 KVP | Catalog mutation, full Esri tile-package/job export semantics, sensor/DEM height mensuration, Esri compact/generated tile-cache management, camera/orientation-ranked find, renderingRule execution, AOI clipping for stats, WMTS |
...
| legend, queryLegends, generateKml, tile | Implemented | Legend swatch images at both legend routes; `f=kml`/`f=kmz`; dynamic PNG tiles at `.../MapServer/tile/{z}/{y}/{x}` use configured cloud file storage (`local`, S3, Azure Blob) as a deterministic loose-object read-through/write-through cache. |
...
| tile, computeHistograms, getSamples | Implemented | PNG/JPEG/TIFF tiles, zoom 0–28, multi-raster mosaic. Tile GETs use configured cloud file storage (`local`, S3, Azure Blob) as a deterministic loose-object read-through/write-through cache; getSamples caps at 1000 samples. |
