...
| colormap, `info/*`, imageSupportData, KML image, raster catalog item child resources (`{rasterId}/image`, `{rasterId}/info/*`), rasterFile | Not implemented | The root raster catalog item resource (`{rasterId}`) is implemented; richer child resources remain deferred. |
