...
| [NAServer](#naserver) | Preview (opt-in) | Lifecycle/security-qualified GET/POST route, service-area, closest-facility, OD-cost-matrix (cost-only), and location-allocation (minimize-impedance / maximize-coverage) solves over the shared routing provider | Broader solver parity, OD-matrix geometry output, additional location-allocation problem types, and topology editing are deferred to 2026.2 |
...
| [VersionManagementServer](#versionmanagementserver-branch-versioning) | Preview (opt-in) | Lifecycle/security-qualified version list/info, owner-bound create/alter/delete, reconcile/post, conflict inspection/resolution, and async job polling | Broader Esri session/parity work is deferred to 2026.2; Postgres-only mutations remain explicit. Every route 404s unless the `versioning.branch` Preview capability is enabled. |
...
> **Preview in 2026.1, off by default.** Every route below is gated by the `versioning.branch`
> capability and returns **404** unless `Capabilities:Experimental:versioning.branch:Enabled=true`
> (or the global experimental switch) is set. It is **not a GA claim**; broader parity is
> deferred to 2026.2. The `"maturity": "preview"` on each operation in the JSON is derived from
