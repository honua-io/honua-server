...
| ArcGIS SQL `where` constructs | Implemented (allowlisted) | Comparison/logical operators (`AND`/`OR`/`NOT`), parentheses, `IN`/`NOT IN`, `BETWEEN`/`NOT BETWEEN`, `LIKE`/`NOT LIKE`, `IS [NOT] NULL`, arithmetic (`+ - * / %`), date literals (`#…#`, `DATE '…'`, `TIMESTAMP '…'`), `CURRENT_DATE`/`CURRENT_TIMESTAMP`/`CURRENT_TIME`, ANSI string concatenation `\|\|`, `EXTRACT(YEAR\|MONTH\|DAY\|HOUR\|MINUTE\|SECOND FROM expr)`, `CAST(expr AS type)` (plus the `CAST(expr, 'type')` function form), and the allowlisted scalar function set (string: `UPPER`/`LOWER`/`LENGTH`/`TRIM`/`LTRIM`/`RTRIM`/`SUBSTRING`/`SUBSTR`/`REPLACE`/`CONCAT`/`POSITION`; numeric: `ABS`/`CEIL`/`FLOOR`/`ROUND`/`POWER`/`MOD`/`SQRT`/`SIN`/`COS`/`TAN`/`LOG`/`EXP`; date-part: `YEAR`/`MONTH`/`DAY`/`HOUR`/`MINUTE`/`SECOND`/`NOW`; `COALESCE`/`NULLIF`; spatial `ST_*`). All literals are parameterized and only allowlisted constructs reach the database. **Re-deferred (#1660):** `CASE`/`WHEN`/`THEN`/`ELSE`/`END` conditionals (need a new conditional AST node across every provider + filter language), `LIKE … ESCAPE` (needs an escape-clause AST extension), scalar subqueries (intentional — never push client subqueries to the DB), and `EXTRACT` for non-time fields. |
