...
| computeClass (Honua extension) | Implemented (validation only) | Validates `renderingRule` raster-function chains (`Identity`, `Stretch`, `Colormap`, `Clip`, depth ≤ 8) and returns planned execution metadata. `exportImage` and `identify` execute the `Stretch`, `Colormap` (explicit stops or named `ColorrampName`), and `Clip` (including `ClippingType=1` keep-outside) portions of a chain. |
