...
| WMTS | Partial | OGC WMTS 1.0 GetCapabilities/GetTile (KVP + RESTful) at `.../ImageServer/WMTS` for numeric and service-name ImageServer routes. Uses the ImageServer tile/cloud-cache pipeline, advertises `WebMercatorQuad`, serves `image/png` and `image/jpeg` tiles, answers GetFeatureInfo (application/json or text/xml) with the pixel band values at the requested tile pixel, and advertises a `TIME` dimension for temporal layers (GetTile/GetFeatureInfo honour a `TIME` parameter). Non-WebMercator matrix sets and other tile formats (TIFF) are deferred. |
