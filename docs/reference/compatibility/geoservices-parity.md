...
| [Catalog / REST Info](#catalog--rest-info) | Partial | Shared RBAC-filtered `/rest/services` and SOAP `/services` discovery, with SOAP 1.1/1.2 WSDL and REST handoffs | `/rest/info` always reports `isTokenBasedSecurity: false` and omits `tokenServicesUrl`; no named `folders`; `currentVersion` deliberately not advertised |
