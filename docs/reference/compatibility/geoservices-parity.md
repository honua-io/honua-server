...
| [MapServer](#mapserver--wms--wmts) | Partial | Export, identify, find, legend/queryLegends, query, mapLayer + allowlisted workspace (`dataLayer`) dynamicLayers, tiles with cloud-storage cache, storage-backed exportTiles, generateKml, WMS 1.3/1.1.1, WMTS 1.0 | Full Esri tile-package/job export semantics, dynamicLayers joins/query-tables, several child resources; WMTS is WebMercatorQuad-only |
