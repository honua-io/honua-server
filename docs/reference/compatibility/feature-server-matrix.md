...
Canonical GeoServices entry point: [GeoServices REST Parity](geoservices-parity.md)
...
| Query Date Bins | `/rest/services/{serviceName}/FeatureServer/{layerId}/queryDateBins` | GET, POST | Implemented | `GET/POST /rest/services/{serviceId}/FeatureServer/{layerId}/queryDateBins` | Bins features into time intervals on a date/timestamp field. Requires the `temporal.histogram` entitlement per the temporal animation contract (#379). |
...
### Honua spatial analytics extensions (Pro catalog tier)

These four layer-level operations are Honua extensions to the GeoServices REST surface — they have no direct Esri equivalent, since Esri sells the comparable functionality through the separately licensed Spatial Analyst, Network Analyst, and GeoAnalytics Server products. They are gated by active entitlement keys under [ADR-0024](../../internal/contributor/adr/0024-open-core-edition-model.md) (`analytics.clustering`, `analytics.spatial-join`, `analytics.buffer-aggregate`, `analytics.density`) and return HTTP 402 Payment Required with the GeoServices error envelope (`error.code=402`, `message="Payment Required"`, details containing the inactive entitlement key and upgrade message) when the required entitlement is inactive.
...
The analytics routes are mapped unconditionally and reach a PostGIS-backed `ISpatialAnalyticsReader`. On deployments whose feature-store provider does not ship an analytics reader (for example the DuckDB read-only provider or the MySQL/MariaDB read/query-only provider), any of the four endpoints returns HTTP 501 Not Implemented with a `StandardErrorResponse` body (title `Not Implemented`, detail naming the operation) — treat this as a permanent signal, not a transient failure. See [DuckDB provider](../configuration/data-sources/duckdb.md#write-operations-return-errors) and [MySQL/MariaDB provider](../configuration/data-sources/mysql-mariadb.md#supported-operations) for details on which capabilities each read-only provider declares as unsupported.
...
| Quantization | `quantizationParameters` | Partial | Accepted but ignored for `f=json`/`f=geojson` (server-side coordinate quantization to an integer grid + response `transform` block is not implemented for the textual formats). The layer metadata honestly advertises `supportsCoordinatesQuantization=false`, so spec-aware clients do not request a quantized `transform`. Deferred because a correct implementation requires emitting an integer-coordinate geometry encoding plus a `transform` (scale/translate/originPosition) block in the JSON/GeoJSON response models, which are shared with the binary formatters consumed by other protocol surfaces; the cost/risk outweighs the benefit while `f=pbf` already returns a quantized `transform`. |
...
| `fgb` | `application/vnd.flatgeobuf` | FlatGeobuf binary. Requires a store with native FlatGeobuf support; FeatureServer returns HTTP 400 (`Unsupported output format`) when the configured feature store does not implement it (for example the MySQL/MariaDB read/query-only provider). |
...
| `orderByFields` | Implemented. Comma-separated `field [ASC|DESC]` list, validated against the **related** layer schema (unknown/invalid fields are rejected with `400`). Applied as an in-memory ordering over each source object's related records. |
| `returnCountOnly` | Implemented. Each group carries `objectId` + `count` (the total number of matching related records for that source object) and omits `relatedRecords`; the top-level field/geometry schema is suppressed. Pagination (`resultRecordCount`/`resultOffset`) is intentionally not applied to the count so it reflects the full matching set. |
| `f` | Accepted for compatibility but only JSON output is produced; a non-JSON `f` value is rejected with `400`. |
