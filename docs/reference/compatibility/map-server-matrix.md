...
| Generate Renderer | `.../MapServer/{layerId}/generateRenderer` | GET, POST | Implemented | `GET/POST /rest/services/{serviceId}/MapServer/{layerId}/generateRenderer` | Thin adapter that forwards to the FeatureServer generateRenderer handler unchanged. Returns a simple renderer by default, or a class-breaks/unique-value renderer when `classificationDef` is supplied. |
| Query Related Records | `.../MapServer/{layerId}/queryRelatedRecords` | GET, POST | Implemented | `GET/POST /rest/services/{serviceId}/MapServer/{layerId}/queryRelatedRecords` | Thin adapter that forwards to the FeatureServer queryRelatedRecords handler unchanged. Requires `objectIds` and `relationshipId`; emits the Esri top-level field/geometry metadata plus per-group flat `relatedRecords`. |
| Query Attachments | `.../MapServer/{layerId}/queryAttachments` | GET, POST | Implemented | `GET/POST /rest/services/{serviceId}/MapServer/{layerId}/queryAttachments` | Thin adapter that forwards to the FeatureServer queryAttachments handler unchanged. Supports `objectIds`, `returnUrl`, `attachmentTypes`, `keywords`, `size`, and `definitionExpression`; `globalIds` is rejected. |
...
| Generate Renderer (service-level) | `.../MapServer/generateRenderer` | GET, POST | Not implemented | Only the per-layer form (`.../MapServer/{layerId}/generateRenderer`) is implemented. |
...
- generateRenderer, queryRelatedRecords, and queryAttachments adapters (forward to the FeatureServer handlers as-is): [MapServerRequestHandlers.RelatedAndRenderer](../../../src/Honua.Protocols.GeoServices/MapServer/MapServerRequestHandlers.RelatedAndRenderer.cs)
