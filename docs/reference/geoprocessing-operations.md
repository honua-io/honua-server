...
| `conversion.raster-format` | Raster format conversion (GTiff/PNG/JPEG/COG) via real GDAL `gdal_translate`. | `source`, `targetFormat`, `compression` | **native** | job, workflow |
| `conversion.raster-reproject` | Raster CRS conversion via real GDAL `gdalwarp`. | `source`, `targetSrid`, `resampling` | **native** | job, workflow |
