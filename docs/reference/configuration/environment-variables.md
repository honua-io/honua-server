...
| `Database__AdoptSeededSchemaOnFirstMigration` | `false` | For test and certification fixtures that create migration-owned tables before the server's first boot. When `true` and the migration journal (`public.schema_versions`) is empty, the pending migrations adopt complete seed-created tables instead of failing with `SchemaExistsWithoutJournal`. A seed-created table missing a column its migration creates is still rejected before anything runs, and the full schema floor is verified afterwards. Has no effect once any migration is journaled. Leave it unset in production so a database that has lost its journal fails closed. |
