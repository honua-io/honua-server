...
Honua exposes multiple industry-standard geospatial APIs. This page highlights the major protocol families and helps you choose the right surface at a high level. For the exhaustive launch support matrix and caveats, use [MVP Compatibility and Limitations](../reference/compatibility/clients.md).
...
| **Spec plan/apply engine** | REST + gRPC | `/v1/spec/*` + `geospatial.v1.SpecService` | Terraform-style plan/apply with content-hash artifact cache — see [Spec Engine reference](../reference/spec-engine.md) |
...
**Edition gating:** Community-tier, gated by the `identity.portal-token` entitlement (always active in Community mode). The endpoint returns `402 Payment Required` when the entitlement is not active. Configuration knobs (`Authentication:PortalToken:Enabled`, `RequireHttps`, `DefaultExpirationMinutes`, `MaxExpirationMinutes`) are documented in [Security](../guides/secure/authentication.md#authentication).
...
**Contract notes:**
- Scenes are registered through the admin [Scene Dataset Registry](../internal/admin-api/scene-dataset-registry.md) (`/api/v1/admin/scenes`). Each record stores the URL slug `id`, `assetRoot` (filesystem prefix that contains `tileset.json`), optional `tilesetFileName`, `description`, extent/CRS metadata, `cachePolicy`, `editionGate`, and access flags (`isPublic`, `requiresAuth`, `allowedRoles`). The local-dev `Scenes` configuration block is retained as a fallback when Postgres is unavailable.
...
See [Hosted 3D Tiles Scenes](../guides/publish/publish-3d-scenes.md) for configuration, the CesiumJS usage example, and the full asset-resolution contract.
...
- Generation failures surface stable problem-detail codes: `SCENE_LAYER_NOT_FOUND` (404), `SCENE_LAYER_CRS_UNKNOWN` (400), `SCENE_UNSUPPORTED_GEOMETRY_TYPE` (400), `SCENE_FEATURE_LIMIT_EXCEEDED` (400), `SCENE_ATTRIBUTE_TYPE_UNSUPPORTED` (400, reserved), `SCENE_MODEL_ASSET_INVALID` (400, reserved), `SCENE_REGISTRATION_CONFLICT` (409), `SCENE_OPTIONS_INVALID` (400). `503` is returned for transient executor-side failures.
...
See [3D Tiles Generation Pipeline](../guides/publish/scene-generation.md) for the full request/response contract, configuration keys, and deferred enterprise-scale work list.
...
- See [ADR-0029](../internal/contributor/adr/0029-geoprocess-canonical-model-mappings.md) for adapter invariants and the [Geoprocess Framework Analysis](../guides/query-analyze/run-geoprocessing.md) for the full canonical model mapping.
...
The `?theme` query parameter applies a deterministic transform (dark-mode lightness inversion, colorblind-safe palette remap, or solid high-contrast print). The output cache varies per theme; admin style writes invalidate every variant. See [Style Engine: Cross-Protocol Consumption](../guides/style/style-maps.md) for the full theme contract and how the same canonical document drives MapServer, WMS, and FeatureServer consumption.
...
- **Client template validation** via the [Client Templates + Manual Smoke Runbook](../gis/CLIENT_TEMPLATE_RUNBOOK.md).
...
Removal of previously supported standards API operations follows the same deprecation lifecycle as control-plane APIs: a minimum of **90 calendar days** and at least **2 minor releases**, whichever is longer. See [CONTROL_PLANE_VERSIONING_POLICY.md](../reference/versioning-and-support.md#deprecation-lifecycle) for the full lifecycle.
...
- [GeoServices REST Parity Data (JSON)](../gis/data/geoservices-rest-parity.json) — machine-readable export of the same operation and parameter contract
- [FeatureServer Coverage Matrix](../reference/compatibility/feature-server-matrix.md) — aligned to [Esri REST Feature Service spec](https://developers.arcgis.com/rest/services-reference/enterprise/feature-service/)
- [MapServer Coverage Matrix](../reference/compatibility/map-server-matrix.md) (includes WMS 1.3 and WMTS 1.0) — aligned to [Esri REST Map Service spec](https://developers.arcgis.com/rest/services-reference/enterprise/map-service/)
- [ImageServer Coverage Matrix](../reference/compatibility/image-server-matrix.md) — aligned to [Esri REST Image Service spec](https://developers.arcgis.com/rest/services-reference/enterprise/image-service/)
- [Geometry Service Matrix](../reference/compatibility/geometry-service-matrix.md) — buffer, simplify, project, intersect, union, clip, difference, plus Honua supplemental `area`/`length` routes
- [Geoprocess Framework Analysis](../guides/query-analyze/run-geoprocessing.md) — GPServer canonical model mapping, lifecycle state matrix, and adapter invariants

**OGC API:**
- [OGC API Features Coverage](../reference/protocols/specifications/ogc-api-features-coverage.md)
  - [Part 1 — Core](../reference/protocols/specifications/ogc-api-features-part1-core.md)
  - [Part 2 — CRS](../reference/protocols/specifications/ogc-api-features-part2-crs.md)
  - [Part 3 — Filtering](../reference/protocols/specifications/ogc-api-features-part3-filtering.md)
- [OGC API Tiles Coverage](../reference/protocols/specifications/ogc-api-tiles-coverage.md)
- [OGC API Processes Coverage](../reference/protocols/specifications/ogc-api-processes-coverage.md)

**OData v4:**
- [OData v4 Coverage](../reference/protocols/specifications/odata-v4-coverage.md)

**Public interface governance:**
- [Public Interface Proof Ledger (JSON)](../gis/data/public-interface-proof.json) — canonical machine-readable inventory of every shipped surface, proof classes, CI lanes, and evidence locations
- [Public Interface Quality Model](../internal/contributor/public-interface-quality-model.md) — human-readable explanation of proof classes, release evidence rules, and ticket reconciliation

**Client validation artifacts:**
- [Client Templates + Manual Smoke Runbook](../gis/CLIENT_TEMPLATE_RUNBOOK.md)
...
- [GeoServices REST Parity Data (JSON)](../gis/data/geoservices-rest-parity.json)
- [FeatureServer Coverage Matrix](../reference/compatibility/feature-server-matrix.md)
- [MapServer Coverage Matrix](../reference/compatibility/map-server-matrix.md)
- [ImageServer Coverage Matrix](../reference/compatibility/image-server-matrix.md)
