...
- [Machine-readable SDK compatibility version manifest](../developer/sdk-compatibility-versions.json)
- [SDK Standards Coverage by Language](../internal/developer/SDK_STANDARDS_COVERAGE.md)
...
- [Control Plane Versioning Policy](../reference/versioning-and-support.md)
- [SDK Migration Guide Template](../internal/developer/sdk-migration-template.md)
- [SDK Migration Automation Evidence Manifest](../internal/developer/sdk-migration-evidence-manifest.md)
...
- This matrix versions the control-plane/admin API contract and gates the
  ...
  Use [SDK Standards Coverage by Language](../internal/developer/SDK_STANDARDS_COVERAGE.md) for
...
[`sdk-compatibility-versions.json`](../developer/sdk-compatibility-versions.json). The
manifest defines:
...
## SDK Migration Automation Evidence

Migration automation evidence is recorded in the same cell artifact as the
read/write/auth surfaces but carries its own surface set, status vocabulary,
...
- `supported` - SDK implemented the wrapper and the live smoke flow passed in
  ...
  `compat-result.json`.
- `failed` - SDK implemented the wrapper and the smoke flow ran but failed.
  Requires `failure.log_tail_path` with at least the last 80 lines of the smoke
  log.
- `skipped` - SDK intentionally did not exercise the surface in this cell.

`unsupported` and `failed` look similar in run output but mean different things
for release evidence promotion. `unsupported` is "we did not try"; `failed` is
"we tried and it broke". The matrix decision logic must keep them distinct.

A migration surface counts as release evidence for a given SDK only when every
supported cell for that SDK records `status: "supported"` and `passed: true`
...
6. If the release is breaking or deprecates an operation you use, follow the
   step-by-step process in [Control Plane Migration Guide](../reference/control-plane-migration-guide.md)
