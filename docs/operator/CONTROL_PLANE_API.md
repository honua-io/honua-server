...
- `targetBaseUrl` must also match this server's resolved public base URL. Cross-instance evidence generation is not supported by this endpoint because target-side style and deploy-preflight evidence is gathered from the current server's admin state.
...
- Listing supports non-negative `limit` and `offset`, plus exact filters `provider=arcgis-geoservices`, `cutoverProfile=pilot|production`, and `readiness=blocked|pilot_ready|production_ready`. Results are returned newest-first, summary rows include `reportHash`, `warningCount`, `blockerCount`, `generatedAt`, and the echoed provenance fields (`requestedBy`, `summary`, `inventoryArtifactRef`, `translationManifestRef`, and `importJobId`), and the response echoes the requested pagination values. A `limit` of `0` returns an empty page; otherwise the backing store clamps fetched page size to `0..200` summaries.
