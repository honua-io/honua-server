...
|---------|-------------|-----|
| Jobs stay Queued (local backend) | In-process worker did not start or failed silently | Check geoprocessing submission logs; the local backend executes in-process and the reconciler bridges progress. If the combined host is healthy, the job should transition automatically |
