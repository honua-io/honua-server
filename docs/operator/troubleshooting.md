...
|---------|-------------|-----|
| Jobs stay Queued (local backend) | No worker host running `AddJobWorker()` | Worker-mode hosting is not yet wired; local backend jobs require a future combined or worker-mode host |
