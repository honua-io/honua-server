...
describe("honua_query_features", () => {
  it("returns features with returnedCount", async () => {
    ...
    const result = await execute(
      asClient(mock),
      schema.parse({ serviceId: "Parks", layerId: 0 }),
    );
  ...
  it("maps spatialRel to Esri enum", async () => {
    ...
    await execute(
      asClient(mock),
      schema.parse({ serviceId: "Parks", layerId: 0, spatialRel: "contains" }),
    );

    expect(mock.queryFeatures).toHaveBeenCalledWith(
      expect.objectContaining({ spatialRel: "esriSpatialRelContains" }),
    );
  });
  ...
  it("clamps limit to 2000", async () => {
    ...
    await execute(
      asClient(mock),
      schema.parse({ serviceId: "Parks", layerId: 0, limit: 5000 }),
    );

    expect(mock.queryFeatures).toHaveBeenCalledWith(
      expect.objectContaining({ resultRecordCount: 2000 }),
    );
  });
  ...
  it("defaults limit to 100", async () => {
    ...
    expect(mock.queryFeatures).toHaveBeenCalledWith(
      expect.objectContaining({ resultRecordCount: 100 }),
    );
  });
  ...
  it("maps orderBy to orderByFields", async () => {
    ...
    await execute(
      asClient(mock),
      schema.parse({ serviceId: "Parks", layerId: 0, orderBy: "NAME ASC" }),
    );

    expect(mock.queryFeatures).toHaveBeenCalledWith(
      expect.objectContaining({ orderByFields: "NAME ASC" }),
    );
  });
