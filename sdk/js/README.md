...
- compat widgets/components (`LayerListCompat`, `LegendCompat`, `PopupCompat`, `SearchCompat`, `BasemapGalleryCompat`, `BookmarksCompat`, `ExpandCompat`) backed by a shared `CompatEventBus` so widgets/components can subscribe to layer/view changes,
- common map controls (`HomeCompat`, `BasemapToggleCompat`, `LocateCompat`, `ScaleBarCompat`, `CompassCompat`) wired to the same event bus for shared view state updates,
...
- alternate target (`--target esri-leaflet`) rewrites deterministic subset only:
  ...
  - non-deterministic APIs (for example `Map`, `MapView`, `SceneView`, `WebMap`, `GraphicsLayer`, `GroupLayer`, `LayerList`, `Legend`, `Popup`, `Home`, `BasemapToggle`, `Locate`, `ScaleBar`, `Search`, `BasemapGallery`, `Bookmarks`, `Expand`, `Compass`) are emitted as manual TODO/report entries
